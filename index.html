...
<title>EDL IDF — CRM</title>
<link rel="icon" type="image/svg+xml" href="/favicon.svg">
<link rel="alternate icon" href="/favicon.ico">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@2.44.0/tabler-icons.min.css">
<script>
// Détection booking IMMÉDIATE avant chargement complet
(function(){
  const params = new URLSearchParams(window.location.search);
  const path = window.location.pathname;
  // Flag global extranet — défini immédiatement, avant tout le reste
  window._EXTRANET_MODE = (path === '/extranet' || params.get('extranet') === '1');
  if(window._EXTRANET_MODE){
    document.documentElement.setAttribute('data-mode', 'extranet');
  }
  if(path === '/booking' || params.get('booking') === '1' || path === '/extranet' || params.get('extranet') === '1'){
    document.addEventListener('DOMContentLoaded', function(){
      const authScreen = document.getElementById('auth-screen');
      if(authScreen){ authScreen.style.display='none'; authScreen.classList.remove('show'); }
    });
  }
})();
</script>
...
body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;font-size:14px;color:var(--text);background:var(--bg3);height:100vh;overflow:hidden}
.crm{display:none;height:100vh}
/* SIDEBAR — Design SaaS EDLConnect */
.sidebar{width:220px;min-width:220px;background:#0d1929;border-right:none;display:flex;flex-direction:column;overflow-y:auto}
.logo{padding:16px 14px 12px;border-bottom:1px solid rgba(24,95,165,0.2)}
.logo-top{display:flex;align-items:center;gap:10px;margin-bottom:10px}
.logo-icon{width:32px;height:32px;background:#185FA5;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:13px;font-weight:700;flex-shrink:0}
.logo-title{font-size:13px;font-weight:500;color:#ffffff}
.logo-sub{font-size:11px;color:rgba(255,255,255,0.35);margin-top:1px}
.logo-badges{display:flex;gap:5px;margin-top:0}
.lbadge{font-size:10px;padding:3px 10px;border-radius:20px;font-weight:500}
.lbadge-green{background:rgba(24,95,165,0.2);color:#7bb8ef;border:0.5px solid rgba(55,138,221,0.3)}
.lbadge-blue{background:rgba(55,138,221,0.12);color:#7bb8ef;border:0.5px solid rgba(55,138,221,0.2)}
.lbadge-active-dot{display:inline-block;width:5px;height:5px;background:#378ADD;border-radius:50%;margin-right:4px;vertical-align:middle}
.nav-section{font-size:9px;font-weight:500;color:rgba(255,255,255,0.22);padding:10px 12px 3px;text-transform:uppercase;letter-spacing:.09em}
.nav-item{display:flex;align-items:center;gap:0;padding:0 10px;cursor:pointer;font-size:12.5px;color:rgba(255,255,255,0.45);border:none;background:none;width:100%;text-align:left;transition:background .12s;border-radius:8px;margin:1px 0;height:38px}
.nav-item:hover{background:rgba(24,95,165,0.12);color:rgba(255,255,255,0.75)}
.nav-item.active{background:rgba(24,95,165,0.22);color:#7bb8ef;font-weight:500}
.nav-icon{width:28px;height:28px;border-radius:7px;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-right:10px;font-size:15px}
.nav-item i{font-size:15px}
.nav-badge{margin-left:auto;font-size:10px;padding:1px 7px;border-radius:20px;font-weight:600}
.nb-red{background:rgba(229,62,62,0.18);color:#fc8181;border:0.5px solid rgba(229,62,62,0.3)}
.nb-amber{background:rgba(186,117,23,0.2);color:#f6c05e;border:0.5px solid rgba(186,117,23,0.3)}
.sidebar-footer{margin-top:auto;padding:10px;border-top:1px solid rgba(24,95,165,0.2)}
.sidebar-footer div{font-size:11px;color:rgba(255,255,255,0.25)}
.sync-row{display:flex;align-items:center;gap:5px;margin-bottom:3px}
...
/* BUTTONS */
.btn{display:inline-flex;align-items:center;gap:6px;padding:7px 14px;border-radius:var(--radius);border:1px solid var(--border2);background:var(--bg);cursor:pointer;font-size:12px;color:var(--text);transition:all .12s;font-family:inherit}
...
.kpi{background:var(--bg2);border-radius:var(--radius);padding:14px;border:1px solid var(--border)}
...
/* CARDS */
.card{background:var(--bg);border:1px solid var(--border);border-radius:var(--radius-lg);margin-bottom:14px;overflow:hidden}
.card-head{padding:10px 14px;border-bottom:1px solid var(--border);font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:space-between}
/* TABLE */
.tbl{width:100%;border-collapse:collapse;font-size:12px}
.tbl th{padding:7px 10px;text-align:left;font-size:10px;font-weight:600;color:var(--text2);border-bottom:1px solid var(--border);text-transform:uppercase;letter-spacing:.04em;white-space:nowrap}
.tbl td{padding:8px 10px;border-bottom:1px solid var(--border);vertical-align:middle;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.tbl tr:last-child td{border-bottom:none}
.tbl tr:hover td{background:var(--bg2)}
/* BADGES */
.badge{display:inline-flex;align-items:center;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:500;white-space:nowrap}
...
.b-gray{background:#f1f0ec;color:#555}
.b-teal{background:var(--teal-bg);color:var(--teal-text)}
.b-purple{background:#EEEDFE;color:#3C3489}
/* FORMS */
input,select,textarea{border:1px solid var(--border2);border-radius:var(--radius);padding:7px 10px;font-size:12px;background:var(--bg);color:var(--text);font-family:inherit;outline:none;width:100%}
input:focus,select:focus,textarea:focus{border-color:var(--blue);box-shadow:0 0 0 3px rgba(24,95,165,.12)}
label{font-size:11px;color:var(--text2);display:block;margin-bottom:3px;margin-top:10px}
...
.modal{background:var(--bg);border-radius:var(--radius-lg);padding:22px;width:520px;max-width:95vw;max-height:88vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.2)}
...
.modal-close{cursor:pointer;background:none;border:none;color:var(--text2);font-size:18px;padding:2px;line-height:1}
/* PIPELINE */
.pipe-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:10px;min-height:260px}
.pipe-col{background:var(--bg2);border-radius:var(--radius);padding:10px;border:1px solid var(--border)}
.pipe-col-title{font-size:10px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px;display:flex;justify-content:space-between}
.pipe-card{background:var(--bg);border:1px solid var(--border);border-radius:var(--radius);padding:9px;margin-bottom:7px;cursor:pointer}
.pipe-card:hover{border-color:var(--border2);box-shadow:0 2px 8px rgba(0,0,0,.06)}
.pipe-card-name{font-size:11px;font-weight:600;margin-bottom:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.pipe-card-amount{font-size:12px;font-weight:600;color:var(--blue)}
.pipe-card-meta{font-size:10px;color:var(--text2);margin-top:3px}
/* CALENDAR */
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:3px}
.cal-hdr{font-size:10px;font-weight:600;color:var(--text2);text-align:center;padding:4px}
.cal-day{min-height:64px;border:1px solid var(--border);border-radius:var(--radius);padding:4px;cursor:pointer;transition:background .1s}
...
.cal-ev{font-size:9px;padding:1px 4px;border-radius:3px;margin-bottom:1px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}
.ev-blue{background:var(--blue-bg);color:var(--blue-text)}
.ev-green{background:var(--green-bg);color:var(--green-text)}
.ev-amber{background:var(--amber-bg);color:var(--amber-text)}
/* TABS */
.tabs{display:flex;border-bottom:1px solid var(--border);margin-bottom:14px;gap:0}
.tab{padding:8px 16px;font-size:12px;cursor:pointer;color:var(--text2);border-bottom:2px solid transparent;background:none;border-top:none;border-left:none;border-right:none;font-family:inherit;transition:all .12s}
.tab.active{color:var(--blue);border-bottom-color:var(--blue);font-weight:600}
/* NOTIFICATION */
.notif{position:fixed;bottom:20px;right:20px;background:var(--blue);color:#fff;padding:10px 18px;border-radius:var(--radius);font-size:12px;z-index:999;opacity:0;transition:opacity .3s;pointer-events:none;box-shadow:0 4px 20px rgba(0,0,0,.2)}
.notif.show{opacity:1}
/* MISC */
.dup-row td{background:#FAEEDA !important}
.avatar{width:28px;height:28px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:600;background:var(--blue-bg);color:var(--blue-text);flex-shrink:0}
.flex-row{display:flex;align-items:center;gap:7px}
.search-wrap{position:relative;margin-bottom:12px}
.search-wrap i{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:var(--text2);font-size:15px}
.search-wrap input{padding-left:34px}
.progress-bar{height:5px;background:var(--bg2);border-radius:3px;overflow:hidden;flex:1}
...
.empty{text-align:center;padding:28px;color:var(--text2);font-size:12px}
.compose-grid{display:grid;grid-template-columns:1fr 300px;gap:16px}
.tracking-item{padding:7px 0;border-bottom:1px solid var(--border)}
.tracking-item:last-child{border-bottom:none}
.t-dot{width:7px;height:7px;border-radius:50%;display:inline-block;margin-right:4px}
.filters-row{display:flex;gap:7px;margin-bottom:12px;flex-wrap:wrap}
/* PROSPECTION PIPELINE */
.prosp-col{min-width:150px;background:var(--bg2);border-radius:var(--radius);padding:8px;border:1px solid var(--border);flex-shrink:0}
.prosp-col-title{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px;display:flex;justify-content:space-between;align-items:center}
.prosp-card{background:var(--bg);border:1px solid var(--border);border-radius:var(--radius);padding:8px;margin-bottom:6px;cursor:pointer;transition:all .12s}
.prosp-card:hover{border-color:var(--blue);box-shadow:0 2px 8px rgba(0,0,0,.08)}
.prosp-card-name{font-size:11px;font-weight:600;margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.prosp-card-email{font-size:10px;color:var(--text2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.prosp-card-date{font-size:9px;color:var(--text3);margin-top:2px}
/* SETTINGS */
.settings-section{background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius-lg);padding:16px;margin-bottom:14px}
.settings-title{font-size:13px;font-weight:600;margin-bottom:12px;display:flex;align-items:center;gap:8px}
/* FICHE CONTACT */
.avatar-lg{width:52px;height:52px;border-radius:50%;display:inline-flex;align-items:center;justify-content:center;font-size:18px;font-weight:700;background:var(--blue-bg);color:var(--blue-text);flex-shrink:0}
.fiche-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px}
.fiche-field{background:var(--bg2);border-radius:var(--radius);padding:10px 12px}
.fiche-label{font-size:10px;color:var(--text2);margin-bottom:2px;text-transform:uppercase;letter-spacing:.04em}
.fiche-val{font-size:13px;font-weight:500}
.stat-pill{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:8px;font-size:10px;font-weight:500}
.pill-open{background:var(--green-bg);color:var(--green-text)}
.pill-click{background:var(--blue-bg);color:var(--blue-text)}
.tbl tr.clickable{cursor:pointer}
.tbl tr.clickable:hover td{background:var(--blue-bg) !important;color:var(--blue-text)}

/* ═══════════════════════════════════════════════════════════
   RESPONSIVE — Mobile & Tablette
   ═══════════════════════════════════════════════════════════ */

/* Bouton menu hamburger — caché par défaut (desktop) */
.mobile-menu-btn{
  display:none;
...
@media (max-width:1024px){
  ...
  .kpi-row, .summary-grid{flex-wrap:wrap}
  table{font-size:12px}
}

/* ─── MOBILE & TABLETTE PORTRAIT (≤820px) ───────────────────*/
@media (max-width:820px){
  body{overflow:auto;height:auto;min-height:100vh}
  .crm{flex-direction:column;height:auto;min-height:100vh}
  ...
  .logo{padding-left:60px} /* laisse la place au bouton burger */

  .main{
    width:100%;
    padding-top:54px; /* espace pour le bouton burger flottant */
  }
  .view{padding:14px}
  ...
  /* KPI cards : passer en grille 2 colonnes */
  div[style*="display:flex"][style*="gap:8px"]{flex-wrap:wrap}

  /* Tableaux : scroll horizontal au lieu de débordement cassé */
  .tbl{font-size:11px}
  .tbl, table{
    display:block;overflow-x:auto;-webkit-overflow-scrolling:touch;
    white-space:nowrap;max-width:100%;
  }
  .tbl thead, .tbl tbody, .tbl tr, table thead, table tbody, table tr{display:table-row-group}
  .tbl thead, table thead{display:table-header-group}
  .tbl tr, table tr{display:table-row}

  /* Cartes / grilles génériques en 1 colonne */
  ...
  /* Kanban Pipeline commercial (grid 5 colonnes) → scroll horizontal */
  #pipeline-board.pipe-grid{
    display:flex !important;
    overflow-x:auto;-webkit-overflow-scrolling:touch;
    scroll-snap-type:x proximity;gap:10px;padding-bottom:8px;
  }
  /* Kanban Prospection (déjà en flex avec min-width inline) */
  #prosp-board{
  ...
  .pipe-col, .prosp-col{
    min-width:78vw;scroll-snap-align:start;flex-shrink:0;
  }

  /* Modals : prendre presque tout l'écran, scrollable */
  .modal{
    width:94vw !important;max-width:94vw !important;
    max-height:88vh;overflow-y:auto;padding:16px;
  }

  /* Boutons de la page-header : taille tactile correcte */
  .btn{padding:9px 14px;font-size:13px}
  .btn-sm{padding:7px 11px;font-size:12px}

  /* Sidebar footer : toujours visible en bas du panneau mobile */
  .sidebar-footer{padding-bottom:16px}

  /* Composer : zone de texte plus grande sur mobile */
  textarea{min-height:140px}

  /* Cacher le texte des boutons secondaires si trop serré */
  .page-header select{max-width:100%}
  ...
  /* Agenda : calendrier 7 jours → scroll horizontal */
  .cal-grid{
    display:flex !important;
    overflow-x:auto;-webkit-overflow-scrolling:touch;
    gap:4px;
  }
  .cal-grid > *{min-width:100px;flex-shrink:0}

  /* Dashboard : grilles inline → 1 colonne */
  ...
  /* Tracking emails : colonnes → empilées */
  ...
  /* Boutons d'action dans les tableaux → icônes seules */
  .tbl td:last-child .btn span{display:none}

  /* Composer : zone message pleine largeur */
  #body-f{min-height:200px}

  /* Fiche contact : header en colonne */
  .fiche-header{flex-direction:column !important;align-items:flex-start !important}

  /* Modèles rapides Composer → grille 1 colonne */
...
@media (max-width:480px){
  ...
  .kpi-val, [class*="kpi"] [style*="font-size:24px"]{font-size:20px !important}
  .pipe-col, .prosp-col{min-width:85vw}
  .kpi-grid{grid-template-columns:1fr 1fr !important}
  .main{padding-bottom:70px} /* espace pour la nav bar mobile */
}
...
#mobile-nav-bar button i{font-size:22px}
#mobile-nav-bar button.active{color:var(--blue)}
@media (max-width:820px){
...
@media (max-width: 820px) {
  /* KPI emails : 2 colonnes */
  .kpi-grid[style*="repeat(5"] { grid-template-columns: 1fr 1fr !important; }
  
  /* Missions : masquer colonnes secondaires */
  #missions-table th:nth-child(2), #missions-table td:nth-child(2),
  #missions-table th:nth-child(4), #missions-table td:nth-child(4),
  #missions-table th:nth-child(7), #missions-table td:nth-child(7) { display:none !important; }
  #missions-table { min-width: unset !important; }
  #missions-table td, #missions-table th { padding:5px 4px !important; white-space:normal !important; font-size:10px !important; }

  /* Prospection kanban */
  #prosp-board { overflow-x:auto !important; display:flex !important; scroll-snap-type:x mandatory !important; padding-bottom:8px; }
  .prosp-col { min-width:82vw !important; max-width:82vw !important; scroll-snap-align:start !important; flex-shrink:0 !important; }

  /* Anti-overflow global */
  .view, .card { overflow-x:hidden !important; max-width:100vw !important; }
  .page-header { flex-direction:column !important; align-items:flex-start !important; gap:8px !important; }
  
  /* Contacts : masquer colonnes secondaires */
  #contacts-tbody tr td:nth-child(7), #contacts-tbody tr td:nth-child(8),
  .tbl thead tr th:nth-child(7), .tbl thead tr th:nth-child(8) { display:none !important; }
}

@media (max-width: 480px) {
  .page-header .btn i { margin:0 !important; }
}
/* AUTH */
/* ─── MODE EXTRANET CLIENT (isolé en plein écran, indépendant de toute mise en page parente) ─── */
html[data-mode="extranet"] body { background: linear-gradient(160deg,#0d1929 0%,#1a2d4a 100%) !important; min-height: 100vh; }
html[data-mode="extranet"] #auth-screen{display:none!important}
html[data-mode="extranet"] .sidebar,
html[data-mode="extranet"] #mobile-nav-bar,
html[data-mode="extranet"] #mobile-menu-btn,
html[data-mode="extranet"] .mobile-menu-btn,
html[data-mode="extranet"] .crm{display:none!important}
html[data-mode="extranet"] .view{display:none!important}
html[data-mode="extranet"] #view-extranet{display:block!important;width:100vw!important;max-width:none!important;min-height:100vh!important;position:relative!important;background:transparent!important;overflow-y:auto!important;z-index:10000!important}

#auth-screen{display:none;position:fixed;inset:0;z-index:9999;background:linear-gradient(135deg,#f8f8f6 0%,#e8f0f8 100%);align-items:center;justify-content:center}
#auth-screen.show{display:flex}
.auth-card{background:#fff;border-radius:16px;padding:40px;width:420px;max-width:95vw;box-shadow:0 20px 60px rgba(0,0,0,.12)}
.auth-logo{font-size:28px;font-weight:700;color:var(--blue);margin-bottom:4px}
...
.auth-tab{padding:10px 20px;cursor:pointer;font-size:13px;color:var(--text2);border-bottom:2px solid transparent;background:none;border-top:none;border-left:none;border-right:none;font-family:inherit;transition:all .12s}
.auth-tab.active{color:var(--blue);border-bottom-color:var(--blue);font-weight:600}
.auth-error{background:var(--red-bg);color:var(--red-text);padding:10px 14px;border-radius:var(--radius);font-size:12px;margin-bottom:12px;display:none}
.auth-error.show{display:block}
.auth-success{background:var(--green-bg);color:var(--green-text);padding:10px 14px;border-radius:var(--radius);font-size:12px;margin-bottom:12px;display:none}
...
<div class="crm">
  <!-- SIDEBAR -->
  <button class="mobile-menu-btn" id="mobile-menu-btn" onclick="toggleMobileSidebar()" aria-label="Menu">
  ...
  <div class="sidebar" id="sidebar">
    ...
    <div class="logo">
      <div class="logo-top">
        <div style="width:36px;height:36px;background:#E6F1FB;border-radius:9px;display:flex;flex-direction:column;align-items:center;justify-content:center;flex-shrink:0">
        <div style="font-size:13px;font-weight:700;color:#185FA5;line-height:1">E<span style="font-weight:300;color:#378ADD">)</span></div>
        <div style="font-size:5px;font-weight:700;color:#185FA5;letter-spacing:0.5px;line-height:1">CONNECT</div>
      </div>
        <div>
          <div class="logo-title" id="sidebar-company-name">EDLConnect</div>
          <div class="logo-sub" id="sidebar-user-email">CRM Pro</div>
        </div>
      </div>
      <div class="logo-badges">
        <span class="lbadge lbadge-green"><span class="lbadge-active-dot"></span>Pro · Actif</span>
      </div>
    </div>

    <div style="padding:8px 10px;flex:1;display:flex;flex-direction:column;overflow-y:auto">

      <button class="nav-item active" onclick="nav('dashboard')">
        <div class="nav-icon" style="background:rgba(24,95,165,0.2)"><i class="ti ti-layout-dashboard" style="color:#7bb8ef"></i></div>
        Accueil
      </button>

      <span class="nav-section">Activité</span>
      <button class="nav-item" onclick="nav('reservations')">
        <div class="nav-icon" style="background:rgba(229,62,62,0.12)"><i class="ti ti-inbox" style="color:#fc8181"></i></div>
        Réservations
        <span class="nav-badge nb-red" id="resa-nav-badge" style="display:none"></span>
      </button>
      <button class="nav-item" onclick="nav('missions')">
        <div class="nav-icon" style="background:rgba(56,161,105,0.12)"><i class="ti ti-clipboard-list" style="color:#68d391"></i></div>
        Missions EDL
      </button>
      <button class="nav-item" onclick="nav('agenda')">
        <div class="nav-icon" style="background:rgba(55,138,221,0.12)"><i class="ti ti-calendar" style="color:#7bb8ef"></i></div>
        Agenda
      </button>

      <span class="nav-section">Commercial</span>
      <button class="nav-item" onclick="nav('contacts')">
        <div class="nav-icon" style="background:rgba(24,95,165,0.15)"><i class="ti ti-building-store" style="color:#7bb8ef"></i></div>
        Contacts & Agences
        <span class="nav-badge nb-amber" id="dup-nav-badge" style="display:none"></span>
      </button>
      <button class="nav-item" onclick="nav('prospection')">
        <div class="nav-icon" style="background:rgba(186,117,23,0.12)"><i class="ti ti-target" style="color:#f6c05e"></i></div>
        Prospection
        <span class="nav-badge nb-amber" id="prosp-badge" style="display:none"></span>
      </button>
      <button class="nav-item" onclick="nav('pipeline')">
        <div class="nav-icon" style="background:rgba(55,138,221,0.1)"><i class="ti ti-chart-bar" style="color:#7bb8ef"></i></div>
        Pipeline
      </button>

      <span class="nav-section">Communication</span>
      <button class="nav-item" onclick="nav('compose')">
        <div class="nav-icon" style="background:rgba(24,95,165,0.15)"><i class="ti ti-send" style="color:#7bb8ef"></i></div>
        Composer
      </button>
      <button class="nav-item" onclick="nav('campaigns')">
        <div class="nav-icon" style="background:rgba(55,138,221,0.1)"><i class="ti ti-speakerphone" style="color:#7bb8ef"></i></div>
        Campagnes
      </button>

      <span class="nav-section">Outils</span>
      <button class="nav-item" onclick="nav('brevo')">
        <div class="nav-icon" style="background:rgba(255,255,255,0.05)"><i class="ti ti-refresh" style="color:rgba(255,255,255,0.3)"></i></div>
        Sync Brevo
      </button>
      <button class="nav-item" onclick="nav('settings')">
        <div class="nav-icon" style="background:rgba(255,255,255,0.05)"><i class="ti ti-settings" style="color:rgba(255,255,255,0.3)"></i></div>
        Paramètres
      </button>
      <button class="nav-item" onclick="nav('help')">
        <div class="nav-icon" style="background:rgba(255,255,255,0.05)"><i class="ti ti-help-circle" style="color:rgba(255,255,255,0.3)"></i></div>
        Guide & Aide
      </button>
      <button class="nav-item" id="nav-admin" style="display:none" onclick="nav('admin')">
        <div class="nav-icon" style="background:rgba(186,117,23,0.12)"><i class="ti ti-shield-lock" style="color:#f6c05e"></i></div>
        Admin
      </button>

    </div>

    <div style="padding:10px;border-top:1px solid rgba(24,95,165,0.2)">
      <div style="display:flex;gap:5px;margin-bottom:8px">
        <button class="btn btn-sm" style="flex:1;justify-content:center;font-size:11px;background:rgba(24,95,165,0.15);border:0.5px solid rgba(55,138,221,0.25);color:rgba(255,255,255,0.5);border-radius:8px" onclick="saveBackup()" title="Sauvegarder">
          <i class="ti ti-device-floppy"></i>Sauvegarder
        </button>
        <label class="btn btn-sm" style="flex:1;justify-content:center;font-size:11px;cursor:pointer;margin:0;background:rgba(255,255,255,0.04);border:0.5px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.35);border-radius:8px" title="Restaurer">
          <i class="ti ti-upload"></i>Restaurer
          <input type="file" accept=".json" onchange="restoreBackup(this)" style="display:none">
        </label>
      </div>
      <div id="last-backup-date" style="font-size:10px;color:rgba(255,255,255,0.22);margin-bottom:8px;padding:0 2px"></div>
      <div style="display:flex;align-items:center;gap:8px;padding:6px 6px 2px">
        <div style="width:28px;height:28px;border-radius:50%;background:#185FA5;display:flex;align-items:center;justify-content:center;font-size:11px;color:#fff;font-weight:600;flex-shrink:0" id="sidebar-avatar">TL</div>
        <div style="flex:1;min-width:0">
          <div style="font-size:11px;color:rgba(255,255,255,0.65);font-weight:500;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" id="sidebar-footer-name">Thomas Langlade</div>
          <div style="font-size:10px;color:rgba(255,255,255,0.25);white-space:nowrap;overflow:hidden;text-overflow:ellipsis" id="sidebar-footer-email"></div>
        </div>
        <button onclick="supabaseClient&&supabaseClient.auth.signOut()" style="background:none;border:none;cursor:pointer;padding:4px" title="Déconnexion">
          <i class="ti ti-logout" style="font-size:14px;color:rgba(255,255,255,0.2)"></i>
        </button>
      </div>
      <div class="sync-row" style="padding:4px 6px 0">
        <div class="sync-dot" id="sync-dot"></div>
        <span id="sync-text" style="font-size:10px;color:rgba(255,255,255,0.25)">Synchronisé</span>
      </div>
    </div>
  </div>
  ...
  <div class="main">
    ...
    <div class="view active" id="view-dashboard">
      <div class="page-header">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
          <button class="btn btn-sm" onclick="setDashMonth('all')" id="dash-btn-all" style="background:var(--blue);color:#fff;border-color:var(--blue)">Tout</button>
          <select id="dash-month-select" onchange="setDashMonth(this.value)" style="font-size:12px;padding:5px 10px;border-radius:var(--radius);border:1px solid var(--border2);background:var(--bg);cursor:pointer">
            <option value="all">— Choisir un mois —</option>
          </select>
          <button class="btn btn-sm" onclick="syncAll()"><i class="ti ti-refresh"></i>Sync tout</button>
          <span style="font-size:11px;color:var(--text2)" id="today-label"></span>
        </div>
      </div>
      <div id="dash-period-banner" style="display:none;background:var(--blue-bg);color:var(--blue-text);padding:6px 14px;border-radius:var(--radius);margin-bottom:12px;font-size:12px;font-weight:500"></div>
      <div class="kpi-grid">
        <div class="kpi"><div class="kpi-label">Contacts totaux</div><div class="kpi-value" id="k-contacts">—</div><div class="kpi-sub" id="k-contacts-sub"></div></div>
        <div class="kpi"><div class="kpi-label">Missions</div><div class="kpi-value" id="k-missions">—</div><div class="kpi-sub">ce mois</div></div>
        <div class="kpi"><div class="kpi-label">CA total <span id="ca-mode-label" style="color:var(--blue);cursor:pointer;text-decoration:underline" onclick="toggleTaxMode()" title="Cliquer pour basculer HT/TTC">HT</span></div><div class="kpi-value" id="k-ca">—</div><div class="kpi-sub" id="k-ca-sub"></div></div>
        <div class="kpi"><div class="kpi-label">Doublons</div><div class="kpi-value" id="k-dups" style="color:var(--amber)">—</div><div class="kpi-sub warn" id="k-dups-sub"></div></div>
      </div>
      <!-- KPIs Email Tracking -->
      <div class="kpi-grid" style="grid-template-columns:repeat(5,1fr);margin-top:-6px">
        <div class="kpi" style="border-left:3px solid #888780">
          <div class="kpi-label">📤 Emails envoyés</div>
          <div class="kpi-value" id="k-sent-total">—</div>
          <div class="kpi-sub">total trackés</div>
        </div>
        <div class="kpi" style="border-left:3px solid #3B6D11">
          <div class="kpi-label">👁 Ouverts</div>
          <div class="kpi-value" id="k-opened">—</div>
          <div class="kpi-sub" id="k-opened-pct"></div>
        </div>
        <div class="kpi" style="border-left:3px solid #185FA5">
          <div class="kpi-label">🖱 Cliqués</div>
          <div class="kpi-value" id="k-clicked">—</div>
          <div class="kpi-sub" id="k-clicked-pct"></div>
        </div>
        <div class="kpi" style="border-left:3px solid #854F0B">
          <div class="kpi-label">💬 Réponses</div>
          <div class="kpi-value" id="k-replied">—</div>
          <div class="kpi-sub" id="k-replied-pct"></div>
        </div>
        <div class="kpi" style="border-left:3px solid #A32D2D">
          <div class="kpi-label">😶 Sans suite</div>
          <div class="kpi-value" id="k-no-reply">—</div>
          <div class="kpi-sub" id="k-no-reply-pct"></div>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px">
        <div class="card">
          <div class="card-head">Pipeline commercial</div>
          <div style="padding:12px" id="dash-pipeline"></div>
        </div>
        <div class="card">
          <div class="card-head">Dernières missions</div>
          <table class="tbl"><thead><tr><th>Agence</th><th>Type</th><th>Montant</th><th>Statut</th></tr></thead><tbody id="dash-missions"></tbody></table>
        </div>
        <div class="card">
          <div class="card-head">Contacts récents <span style="font-size:10px;font-weight:400;color:var(--text2)">Notion + Brevo</span></div>
          <table class="tbl"><thead><tr><th>Entreprise</th><th>Email</th><th>Statut</th></tr></thead><tbody id="dash-contacts"></tbody></table>
        </div>
        <div class="card">
          <div class="card-head">Prochains RDV</div>
          <div style="padding:10px" id="dash-rdv"></div>
        </div>
      </div>
    </div>
    ...
    <div class="view" id="view-contacts">
      <div class="page-header">
        <div class="page-title">Contacts & Agences</div>
        <div style="display:flex;gap:7px">
          <button class="btn btn-sm" onclick="filterContactsTab('dups')" id="btn-dups"><i class="ti ti-alert-triangle"></i>Doublons <span id="dup-count-btn"></span></button>
          <button class="btn btn-primary" onclick="openModal('modal-contact')"><i class="ti ti-plus"></i>Nouveau contact</button>
        </div>
      </div>
      <div class="filters-row" id="contact-filter-btns">
        <button class="btn btn-sm active" onclick="filterContactsTab('all',this)">Tous</button>
        <button class="btn btn-sm" onclick="filterContactsTab('Client actif',this)">Clients actifs</button>
        <button class="btn btn-sm" onclick="filterContactsTab('Cible potentielle',this)">Cibles</button>
        <button class="btn btn-sm" onclick="filterContactsTab('brevo',this)">Brevo</button>
        <button class="btn btn-sm" onclick="filterContactsTab('notion',this)">Notion</button>
        <button class="btn btn-sm" onclick="filterContactsTab('dups',this)">Doublons</button>
      </div>
      <div class="search-wrap"><i class="ti ti-search"></i><input type="text" placeholder="Rechercher…" oninput="searchContacts(this.value)"></div>
      <div class="card">
        <div class="card-head"><span id="contacts-count">— contacts</span><span style="font-size:10px;font-weight:400;color:var(--text2)">ligne orange = doublon potentiel</span></div>
        <div style="overflow-x:auto">
          <table class="tbl" style="min-width:680px">
            <thead><tr><th style="width:18%">Entreprise</th><th style="width:12%">Contact</th><th style="width:16%">Email</th><th style="width:10%">Tél</th><th style="width:9%">Type</th><th style="width:9%">Statut</th><th style="width:10%">Dernier contact</th><th style="width:10%">Moyen</th><th style="width:6%"></th></tr></thead>
    ...
    <!-- PROSPECTION -->
    <div class="view" id="view-prospection">
      <div class="page-header">
        <div class="page-title">Pipeline de prospection</div>
        <div style="display:flex;gap:7px">
          <button class="btn btn-sm" onclick="toggleCAPanel()"><i class="ti ti-chart-bar"></i>Analyse CA</button>
          <button class="btn btn-sm" onclick="syncFromBrevoSender()"><i class="ti ti-wand"></i>Sync contacts</button>
          <button class="btn btn-primary" onclick="openModal('modal-prosp')"><i class="ti ti-plus"></i>Ajouter un prospect</button>
        </div>
      </div>

      <!-- PANEL CA ANALYTICS -->
      <div id="ca-panel" style="display:none;margin-bottom:16px">
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:12px">
          <!-- CA Mensuel -->
          <div class="card" style="padding:14px">
            <div style="font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">📅 CA mensuel récurrent</div>
            <div style="font-size:28px;font-weight:700;color:#3B6D11" id="ca-mensuel">—</div>
            <div style="font-size:11px;color:var(--text2);margin-top:2px">€ / mois (clients actifs)</div>
          </div>
          <!-- CA Trimestriel -->
          <div class="card" style="padding:14px">
            <div style="font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">📊 CA trimestriel</div>
            <div style="font-size:28px;font-weight:700;color:#185FA5" id="ca-trim">—</div>
            <div style="font-size:11px;color:var(--text2);margin-top:2px">€ / trimestre</div>
          </div>
          <!-- CA Annuel -->
          <div class="card" style="padding:14px">
            <div style="font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">🏆 CA annuel projeté</div>
            <div style="font-size:28px;font-weight:700;color:#854F0B" id="ca-annuel">—</div>
            <div style="font-size:11px;color:var(--text2);margin-top:2px">€ / an</div>
          </div>
        </div>

        <!-- Détail par client gagné -->
        <div class="card">
          <div class="card-head">
            Détail CA par client gagné
            <span style="font-size:10px;font-weight:400;color:var(--text2)" id="ca-nb-clients"></span>
          </div>
          <div style="overflow-x:auto">
            <table class="tbl">
              <thead>
                <tr>
                  <th style="width:30%">Agence</th>
                  <th style="width:15%">CA mensuel HT</th>
                  <th style="width:12%">TTC</th>
                  <th style="width:15%">CA trimestriel HT</th>
                  <th style="width:12%">TTC</th>
                  <th style="width:13%">CA annuel HT</th>
                  <th style="width:15%">Depuis</th>
          ...
          <div style="padding:10px 14px;border-top:1px solid var(--border);display:flex;gap:20px;background:var(--bg2)">
            <div style="font-size:12px"><strong>TOTAL MENSUEL :</strong> <span id="ca-total-mensuel" style="color:#3B6D11;font-weight:700"></span></div>
            <div style="font-size:12px"><strong>TOTAL ANNUEL :</strong> <span id="ca-total-annuel" style="color:#854F0B;font-weight:700"></span></div>
          </div>
        </div>

        <!-- Objectifs -->
        <div class="card" style="padding:14px;margin-top:12px">
          <div style="font-size:11px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:.05em;margin-bottom:10px">🎯 Objectifs CA</div>
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px">
            <div>
              <div style="font-size:11px;color:var(--text2);margin-bottom:4px">Objectif mensuel (€)</div>
              <input type="number" id="obj-mensuel" placeholder="ex: 5000" style="width:100%" oninput="updateObjectifs()">
            </div>
            <div>
              <div style="font-size:11px;color:var(--text2);margin-bottom:4px">Objectif trimestriel (€)</div>
              <input type="number" id="obj-trim" placeholder="ex: 15000" style="width:100%" oninput="updateObjectifs()">
            </div>
            <div>
              <div style="font-size:11px;color:var(--text2);margin-bottom:4px">Objectif annuel (€)</div>
              <input type="number" id="obj-annuel" placeholder="ex: 60000" style="width:100%" oninput="updateObjectifs()">
            </div>
          </div>
          <div style="margin-top:12px" id="objectifs-progress"></div>
        </div>
      </div>

      <!-- Stats rapides -->
      <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap" id="prosp-stats"></div>

      <!-- Barre de recherche + filtre étape -->
      <div style="display:flex;gap:8px;margin-bottom:12px;align-items:center;flex-wrap:wrap">
        <div class="search-wrap" style="flex:1;min-width:200px;margin-bottom:0">
          <i class="ti ti-search"></i>
          <input type="text" id="prosp-search" placeholder="Rechercher agence, contact, email…"
            oninput="debounceAC(filterProspection,this.value)" autocomplete="off">
        </div>
        <select id="prosp-stage-filter" onchange="filterProspection(document.getElementById('prosp-search').value)"
          style="font-size:12px;padding:7px 10px;border-radius:var(--radius);border:1px solid var(--border2);background:var(--bg);min-width:160px">
        ...
        <button class="btn btn-sm" onclick="clearProspSearch()" id="prosp-clear-btn" style="display:none">
          <i class="ti ti-x"></i> Effacer
        </button>
        <span id="prosp-search-count" style="font-size:11px;color:var(--text2)"></span>
      </div>

      <!-- Kanban -->
      <div style="overflow-x:hidden;padding-bottom:8px">
       <div style="display:flex;gap:10px;overflow-x:auto;-webkit-overflow-scrolling:touch;scroll-snap-type:x mandatory;" id="prosp-board"></div>
      </div>
    </div>
    ...
    <div class="view" id="view-pipeline">
      <div class="page-header">
        <div class="page-title">Pipeline commercial</div>
        <button class="btn btn-primary" onclick="openModal('modal-deal')"><i class="ti ti-plus"></i>Nouvelle opportunité</button>
      </div>
      <div class="pipe-grid" id="pipeline-board"></div>
    </div>

    <div class="view" id="view-missions">
      <div class="page-header">
        <div class="page-title">Missions EDL</div>
        <div style="display:flex;gap:8px">
          <button class="btn" onclick="openParticulierInvoiceModal()"><i class="ti ti-file-invoice"></i>Facture particulier</button>
          <button class="btn" onclick="openMonthlyInvoiceModal()"><i class="ti ti-calendar-stats"></i>Facture mensuelle</button>
          <button class="btn btn-primary" onclick="openNewMissionModal()"><i class="ti ti-plus"></i>Nouvelle mission</button>
        </div>
      </div>
      <div class="filters-row" id="mission-filter-btns">
        <button class="btn btn-sm active" onclick="filterMissions('all',this)">Toutes</button>
        <button class="btn btn-sm" onclick="filterMissions('planifiée',this)">Planifiées</button>
        <button class="btn btn-sm" onclick="filterMissions('en cours',this)">En cours</button>
        <button class="btn btn-sm" onclick="filterMissions('terminée',this)">Terminées</button>
        <button class="btn btn-sm" onclick="filterMissions('facturée',this)">Facturées</button>
      </div>
      <div class="card">
        <div style="overflow-x:auto">
          <table class="tbl" id="missions-table">
            <thead><tr><th>Agence</th><th>Adresse</th><th>Type</th><th>Bien</th><th>Date</th><th>Montant HT</th><th>TVA 20%</th><th>TTC</th><th>Statut</th><th></th><th></th></tr></thead>
    ...
    <div class="view" id="view-campaigns">
      <div class="page-header">
        ...
        <button class="btn btn-primary" onclick="nav('compose')"><i class="ti ti-plus"></i>Nouvelle campagne</button>
      </div>
      <div class="kpi-grid" style="grid-template-columns:repeat(3,1fr)">
        <div class="kpi"><div class="kpi-label">Emails envoyés</div><div class="kpi-value" id="k-sent">0</div></div>
        <div class="kpi"><div class="kpi-label">Taux d'ouverture</div><div class="kpi-value" id="k-open">0%</div></div>
        <div class="kpi"><div class="kpi-label">Taux de clic</div><div class="kpi-value" id="k-click">0%</div></div>
      </div>
      <div class="card">
        <table class="tbl">
          <thead><tr><th>Campagne</th><th>Envoyés</th><th>Ouverts</th><th>Clics</th><th>Réponses</th><th>Date</th><th>Statut</th></tr></thead>
    ...
    <div class="view" id="view-compose">
      <div class="page-header"><div class="page-title">Composer un email</div></div>
      <div class="compose-grid">
        <div class="card" style="padding:16px">
          <div class="tabs">
            <button class="tab active" onclick="composeTab('single',this)">Email unique</button>
            <button class="tab" onclick="composeTab('campaign',this)">Campagne</button>
          </div>
          <div id="compose-single">
            <label>De</label><input value="contact@edl-idf.com" readonly>
            <label>À</label>
            <div style="position:relative">
              <input id="to-f" placeholder="Nom ou email du contact…" oninput="debounceAC(autocompleteContact,this.value)" onblur="setTimeout(()=>document.getElementById('to-suggest').style.display='none',200)" autocomplete="off">
              <div id="to-suggest" style="display:none;position:absolute;top:100%;left:0;right:0;background:var(--bg);border:1px solid var(--border2);border-radius:var(--radius);z-index:50;max-height:200px;overflow-y:auto;box-shadow:0 4px 12px rgba(0,0,0,.1)"></div>
            </div>
            <label>Objet</label><input id="subj-f" placeholder="Objet…">
            <label>Message</label><textarea id="body-f" style="min-height:200px;resize:vertical"></textarea>

            <!-- Pièce jointe -->
            <div style="margin-top:10px">
              <input type="file" id="attach-input" style="display:none" onchange="handleAttachment(event)" accept=".pdf,.doc,.docx,.png,.jpg,.jpeg,.xlsx,.xls">
              <div id="attach-zone" onclick="document.getElementById('attach-input').click()"
                style="border:1.5px dashed var(--border2);border-radius:var(--radius);padding:10px 14px;display:flex;align-items:center;gap:10px;cursor:pointer;background:var(--bg2);transition:border-color .15s"
                onmouseover="this.style.borderColor='var(--blue)'" onmouseout="this.style.borderColor='var(--border2)'">
                <i class="ti ti-paperclip" style="font-size:16px;color:var(--text2)"></i>
                <div id="attach-info" style="flex:1">
                ...
                <button id="attach-clear-btn" onclick="clearAttachment(event)" style="display:none;background:none;border:none;cursor:pointer;color:var(--red);font-size:13px;padding:2px 6px">✕</button>
              </div>
            </div>

            <!-- Panneau IA Claude -->
            <div id="claude-panel" style="display:none;margin-top:12px;border:1px solid #c3dafe;border-radius:var(--radius);padding:12px;background:#eef4ff">
              <div style="font-size:12px;font-weight:600;color:#2345d4;margin-bottom:8px">✨ Rédiger avec Mistral IA</div>
              <label style="font-size:11px;color:var(--text2)">Décris ce que tu veux dire (ex: relance prospect qui n'a pas répondu, proposer rdv)</label>
              <textarea id="claude-prompt" rows="3" style="margin-top:5px;font-size:12px" placeholder="Ex: Email de relance pour une agence immobilière qui n'a pas répondu à mon premier contact, ton professionnel et bienveillant, proposer un appel découverte de 15 min..."></textarea>
              <div style="display:flex;gap:6px;margin-top:8px;align-items:center">
                <button class="btn btn-primary btn-sm" onclick="generateWithClaude()" id="claude-gen-btn" style="font-size:12px">
                  <i class="ti ti-sparkles"></i><span id="claude-gen-label">Générer</span>
                </button>
                <button class="btn btn-sm" onclick="document.getElementById('claude-panel').style.display='none'" style="font-size:12px">Fermer</button>
                <span id="claude-gen-status" style="font-size:11px;color:var(--text2)"></span>
              </div>
            </div>
            <div style="display:flex;gap:7px;margin-top:12px;flex-wrap:wrap">
              <button class="btn btn-primary" onclick="sendEmail()"><i class="ti ti-send"></i>Envoyer via Brevo</button>
              <button class="btn" onclick="applyTpl('cold')"><i class="ti ti-template"></i>Modèle</button>
              <button class="btn" onclick="toggleClaudePanel()" style="background:linear-gradient(135deg,#667eea,#764ba2);color:#fff;border:none">
                <i class="ti ti-sparkles"></i>Rédiger avec IA
              </button>
            </div>
          </div>
          <div id="compose-campaign" style="display:none">
            <label>Nom de la campagne</label><input id="camp-name" placeholder="Prospection Essonne Juin 2026">
            <label>Segment</label>
            <select id="camp-seg"><option>Tous les contacts</option><option>Cibles potentielles</option><option>Clients actifs</option><option>Contacts Brevo</option></select>
            <label>Objet</label><input id="camp-subj" placeholder="Objet…">
            <label>Message</label><textarea id="camp-body" style="min-height:160px;resize:vertical"></textarea>
            <button class="btn btn-primary" style="margin-top:12px" onclick="sendCampaign()"><i class="ti ti-send"></i>Créer la campagne</button>
          </div>
        </div>
        <div>
          <div class="card" style="margin-bottom:10px">
            <div class="card-head">Modèles rapides</div>
            <div style="padding:8px;display:flex;flex-direction:column;gap:4px">
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('intro')"><i class="ti ti-building-store"></i>Présentation EDL IDF</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('cold')"><i class="ti ti-mail"></i>Prospection à froid</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('followup')"><i class="ti ti-refresh"></i>Relance J+2</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('devis')"><i class="ti ti-receipt"></i>Devis</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('confirm_pec')"><i class="ti ti-check"></i>Confirmation prise en charge</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('confirm_entrant')"><i class="ti ti-door-enter"></i>Confirmation EDL entrant</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('confirm_sortant')"><i class="ti ti-door-exit"></i>Confirmation EDL sortant</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('remerciement')"><i class="ti ti-heart"></i>Remerciement après mission</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('partenariat')"><i class="ti ti-handshake"></i>Proposition partenariat</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('avis_google')"><i class="ti ti-star"></i>Avis Google post-prestation</button>
              <button class="btn btn-sm" style="justify-content:flex-start" onclick="applyTpl('summer')"><i class="ti ti-sun"></i>Offre estivale</button>
            </div>
          </div>
          <div class="card">
            <div class="card-head">Tracking emails</div>
            <div style="padding:10px" id="tracking-list"><div class="empty">Aucun email envoyé</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- BREVO SYNC -->
    <div class="view" id="view-brevo">
      <div class="page-header">
        <div class="page-title">Synchronisation Brevo</div>
        <button class="btn btn-primary" onclick="loadBrevo()"><i class="ti ti-refresh"></i>Lancer la sync</button>
      </div>
      <div class="kpi-grid">
        <div class="kpi"><div class="kpi-label">Contacts Brevo</div><div class="kpi-value" id="bk-brevo">—</div></div>
        <div class="kpi"><div class="kpi-label">Contacts Notion</div><div class="kpi-value" id="bk-notion">—</div></div>
        <div class="kpi"><div class="kpi-label">Unifiés</div><div class="kpi-value" id="bk-total">—</div></div>
        <div class="kpi"><div class="kpi-label">Doublons</div><div class="kpi-value" id="bk-dups" style="color:var(--amber)">—</div></div>
      </div>
      <div id="brevo-status" style="padding:20px;text-align:center;color:var(--text2);font-size:13px">Clique sur "Lancer la sync" pour charger tes contacts Brevo</div>
      <div class="card" id="brevo-table-wrap" style="display:none">
        <div class="card-head"><span id="brevo-count"></span>
          <button class="btn btn-success btn-sm" onclick="mergeAllDups()"><i class="ti ti-git-merge"></i>Fusionner tous les doublons</button>
        </div>
        <div style="overflow-x:auto">
          <table class="tbl" style="min-width:700px">
            <thead><tr><th>Nom / Entreprise</th><th>Email</th><th>Téléphone</th><th>Ville</th><th>Statut email</th><th>Ouvertures</th><th>Clics</th></tr></thead>
            <tbody id="brevo-tbody"></tbody>
          </table>
        </div>
      </div>
      <div class="card" id="dups-wrap" style="display:none">
        <div class="card-head" style="background:var(--amber-bg)"><span style="color:var(--amber-text)"><i class="ti ti-alert-triangle"></i> Doublons détectés</span></div>
        <table class="tbl"><thead><tr><th>Contact 1</th><th>Contact 2</th><th>Type</th><th></th></tr></thead><tbody id="dups-tbody"></tbody></table>
      </div>
    </div>
    ...
    <div class="view" id="view-agenda">
      <div class="page-header">
        ...
        <button class="btn btn-primary" onclick="openModal('modal-rdv')"><i class="ti ti-plus"></i>Nouveau RDV</button>
      </div>
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;flex-wrap:wrap">
        <button class="btn btn-sm" onclick="chPeriod(-1)"><i class="ti ti-chevron-left"></i></button>
        <div style="font-size:15px;font-weight:600;min-width:200px;text-align:center" id="cal-label"></div>
        <button class="btn btn-sm" onclick="chPeriod(1)"><i class="ti ti-chevron-right"></i></button>
        <button class="btn btn-sm" onclick="goToday()">Aujourd'hui</button>
        <div style="margin-left:auto;display:flex;gap:6px">
          <button class="btn btn-sm" id="btn-vue-semaine" onclick="setVue('semaine')" style="background:var(--blue);color:#fff;border-color:var(--blue)">7 jours</button>
          <button class="btn btn-sm" id="btn-vue-mois" onclick="setVue('mois')">Mois</button>
        </div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 260px;gap:14px">
        <div>
          <div class="cal-grid" id="cal-headers"></div>
          <div id="cal-body" style="margin-top:3px"></div>
        </div>
        <div class="card">
          <div class="card-head" id="rdv-list-title">Événements de la semaine</div>
          <div style="padding:8px;overflow-y:auto;max-height:440px" id="rdv-list"></div>
        </div>
      </div>
    </div>
    <!-- AIDE -->
    <div class="view" id="view-help">
  <div class="page-header"><div class="page-title">📚 Guide & Aide</div></div>
  <div class="search-wrap" style="margin-bottom:18px">
    <i class="ti ti-search"></i>
    <input type="text" placeholder="Rechercher dans l'aide…" oninput="searchHelp(this.value)" id="help-search">
  </div>
  <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:20px">
    <button class="btn btn-sm" onclick="scrollToHelp('h-dashboard')">📊 Dashboard</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-contacts')">👥 Contacts</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-prospection')">🎯 Prospection</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-missions')">📋 Missions</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-reservations')">📥 Réservations</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-extranet')">🔑 Extranet</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-agents')">👤 Agents</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-facturation')">📄 Facturation</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-emails')">✉️ Emails</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-agenda')">📅 Agenda</button>
    <button class="btn btn-sm" onclick="scrollToHelp('h-faq')">❓ FAQ</button>
  </div>
  <div id="help-content">

  <div class="settings-section" id="h-dashboard">
    <div class="settings-title">📊 Dashboard</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Vue d'ensemble de toute votre activité commerciale.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>KPIs principaux</strong> : Contacts, Missions, CA HT/TTC (cliquer pour basculer), Doublons<br>
      <strong>Email tracking</strong> : Taux d'ouverture, clics, réponses, sans suite<br>
      <strong>Filtrer par mois</strong> : menu déroulant en haut pour analyser une période précise<br>
      <strong>Sync tout</strong> : synchronise contacts Brevo + cloud Supabase
    </div>
    <div style="margin-top:10px;background:var(--green-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--green-text)">
      ✅ Astuce : Cliquer sur HT/TTC dans le KPI CA pour basculer entre les deux affichages.
    </div>
  </div>

  <div class="settings-section" id="h-contacts">
    <div class="settings-title">👥 Contacts & Agences</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Centralisez tous vos contacts professionnels.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Ajouter</strong> → bouton "+ Nouveau contact"<br>
      <strong>Ouvrir une fiche</strong> → cliquer sur une ligne (4 onglets : Infos, Commandes, Emails, Modifier)<br>
      <strong>Filtrer</strong> → Tous / Clients actifs / Cibles / Brevo / Notion / Doublons<br>
      <strong>Rechercher</strong> → barre de recherche (nom, email, entreprise)<br>
      <strong>Envoyer un email</strong> → icône ✉️ à droite de chaque ligne
    </div>
    <div style="margin-top:10px;background:var(--blue-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--blue-text)">
      ℹ️ Le statut passe automatiquement en "Client actif" quand vous créez une mission liée au contact.
    </div>
  </div>

  <div class="settings-section" id="h-prospection">
    <div class="settings-title">🎯 Pipeline de Prospection</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Suivez vos prospects à travers 8 étapes.</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:10px">
      <div style="background:#F1F0EC;border-left:3px solid #888780;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#888780;font-weight:500">À contacter</div>
      <div style="background:#E6F1FB;border-left:3px solid #185FA5;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#185FA5;font-weight:500">Email envoyé</div>
      <div style="background:#EAF3FB;border-left:3px solid #378ADD;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#378ADD;font-weight:500">Email ouvert</div>
      <div style="background:#EAF3DE;border-left:3px solid #639922;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#639922;font-weight:500">Réponse reçue</div>
      <div style="background:#FAEEDA;border-left:3px solid #854F0B;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#854F0B;font-weight:500">RDV planifié</div>
      <div style="background:#EEEDFE;border-left:3px solid #5B3DA5;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#5B3DA5;font-weight:500">Devis envoyé</div>
      <div style="background:#D6EDCA;border-left:3px solid #3B6D11;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#3B6D11;font-weight:500">Gagné ✅</div>
      <div style="background:#FCEBEB;border-left:3px solid #A32D2D;padding:6px 8px;border-radius:var(--radius);font-size:11px;color:#A32D2D;font-weight:500">Perdu ❌</div>
    </div>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Déplacer</strong> → boutons → sur chaque carte prospect<br>
      <strong>Modifier</strong> → cliquer sur la carte<br>
      <strong>Supprimer</strong> → bouton ✕ en haut à droite de la carte<br>
      <strong>Analyse CA</strong> → bouton en haut pour voir le CA des prospects Gagnés
    </div>
    <div style="margin-top:10px;background:var(--green-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--green-text)">
  ...
  <div class="settings-section" id="h-missions">
    <div class="settings-title">📋 Missions EDL</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Planifiez et suivez tous vos états des lieux.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Créer</strong> → "+ Nouvelle mission" → agence, type, adresse, date, montant<br>
      <strong>Modifier</strong> → icône ✏️ à droite de la ligne<br>
      <strong>Supprimer</strong> → icône 🗑️ à droite de la ligne<br>
      <strong>Changer le statut</strong> → menu déroulant directement dans le tableau<br>
      <strong>Générer une facture</strong> → icône 📄 à droite de la ligne
    </div>
    <div style="margin-top:10px;display:grid;grid-template-columns:1fr 1fr;gap:6px">
      <div style="background:var(--blue-bg);padding:8px;border-radius:var(--radius);font-size:11px;color:var(--blue-text)">📅 <strong>Planifiée</strong> — Mission programmée</div>
      <div style="background:var(--amber-bg);padding:8px;border-radius:var(--radius);font-size:11px;color:var(--amber-text)">⏳ <strong>En cours</strong> — En réalisation</div>
      <div style="background:var(--teal-bg);padding:8px;border-radius:var(--radius);font-size:11px;color:var(--teal-text)">🏁 <strong>Terminée</strong> — Non facturée</div>
      <div style="background:var(--green-bg);padding:8px;border-radius:var(--radius);font-size:11px;color:var(--green-text)">✅ <strong>Facturée</strong> — Facture générée</div>
    </div>
  </div>

  <div class="settings-section" id="h-facturation">
    <div class="settings-title">📄 Facturation</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Générez des factures PDF professionnelles automatiquement.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Par mission</strong> → icône 📄 à droite de chaque mission<br>
      <strong>Mensuelle groupée</strong> → bouton "Facture mensuelle" → saisir client + mois<br>
      <strong>Particulier</strong> → bouton "Facture particulier" → saisir les infos<br>
      <strong>Re-télécharger</strong> → fiche contact → onglet Commandes → bouton ⬇️
    </div>
    <div style="margin-top:10px;background:var(--amber-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--amber-text)">
      ⚠️ Configurez vos infos dans Paramètres → Facturation avant la première facture (SIRET, IBAN, logo…)
    </div>
  </div>

  <div class="settings-section" id="h-emails">
    <div class="settings-title">✉️ Emails & Campagnes</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Envoyez et trackez vos emails via Brevo.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Envoyer</strong> → Composer → destinataire → message → "Envoyer via Brevo"<br>
      <strong>Modèles</strong> → 11 modèles disponibles (bouton "Modèle" ou liste à droite)<br>
      <strong>IA</strong> → bouton "✨ Rédiger avec IA" → décrire l'email souhaité<br>
      <strong>Annuler</strong> → bouton Annuler dans le toast (15 secondes)<br>
      <strong>Tracking</strong> → liste à droite du Composer + fiche contact → onglet Emails
    </div>
    <div style="margin-top:10px;background:var(--green-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--green-text)">
  ...
  <div class="settings-section" id="h-agenda">
    <div class="settings-title">📅 Agenda</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Visualisez RDV et missions dans un calendrier.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Vue semaine</strong> → 7 jours glissants, navigation ← →<br>
      <strong>Vue mois</strong> → grille mensuelle complète<br>
      <strong>Nouveau RDV</strong> → bouton "+ Nouveau RDV" → titre, date, durée, type<br>
      <strong>Google Agenda</strong> → icône 📅 sur chaque événement pour l'ajouter en 1 clic<br>
      <strong>Aujourd'hui</strong> → bouton pour revenir à la période actuelle
    </div>
    <div style="margin-top:10px;background:var(--blue-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--blue-text)">
      ℹ️ Missions en <span style="color:var(--green);font-weight:600">vert</span>, RDV en <span style="color:var(--blue);font-weight:600">bleu</span> dans le calendrier.
    </div>
  </div>

  <div class="settings-section" id="h-reservations">
    <div class="settings-title">📥 Réservations entrantes</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Recevez et gérez les demandes d'état des lieux de vos agences clientes.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Lien de réservation</strong> → à communiquer à chaque agence : <code>app.edlconnect.fr/booking?agency=XXX</code><br>
      <strong>Nouvelle demande</strong> → apparaît automatiquement dans l'onglet Réservations avec une alerte sonore<br>
      <strong>Confirmer un RDV</strong> → bouton "Confirmer" → choisir date, heure, agent → envoie la convocation au(x) locataire(s)<br>
      <strong>Importer en mission</strong> → bouton "Importer" → crée automatiquement une mission dans l'onglet Missions<br>
      <strong>Plusieurs locataires</strong> → le formulaire permet d'ajouter plusieurs locataires, chacun reçoit sa convocation<br>
      <strong>Actualisation auto</strong> → toutes les 2 minutes, avec notification push si activée
    </div>
    <div style="margin-top:10px;background:var(--blue-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--blue-text)">
  ...
  <div class="settings-section" id="h-extranet">
    <div class="settings-title">🔑 Extranet client</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Offrez à vos agences clientes un espace personnel pour suivre leurs commandes.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Activer pour un client</strong> → passer le contact en statut "Client signé ✅" → il reçoit un email de bienvenue avec son lien<br>
      <strong>Lien d'accès</strong> → <code>app.edlconnect.fr/extranet</code> → connexion par lien magique (sans mot de passe)<br>
      <strong>Ce que voit le client</strong> → ses commandes avec statuts, statistiques annuelles, formulaire nouvelle commande, documents<br>
      <strong>Ajouter des documents</strong> → fiche contact → onglet "Modifier" → section "Documents extranet" → coller l'URL Google Drive<br>
      <strong>Statuts synchronisés</strong> → quand vous confirmez un RDV, le client voit "Confirmé" dans son espace automatiquement
    </div>
    <div style="margin-top:10px;background:var(--green-bg);border-radius:var(--radius);padding:10px;font-size:11px;color:var(--green-text)">
  ...
  <div class="settings-section" id="h-agents">
    <div class="settings-title">👤 Agents EDL</div>
    <p style="font-size:12px;color:var(--text2);margin-bottom:10px">Gérez les experts qui réalisent les états des lieux sur le terrain.</p>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Ajouter un agent</strong> → Paramètres → section "Agents EDL" → nom + mobile → "Ajouter cet agent"<br>
      <strong>Assigner à un RDV</strong> → lors de la confirmation d'un RDV → menu "Expert EDL qui se déplace"<br>
      <strong>Dans la convocation</strong> → le nom et le mobile de l'agent apparaissent automatiquement dans l'email du locataire
    </div>
  </div>

  <div class="settings-section" id="h-faq">
    <div class="settings-title">❓ FAQ</div>
    <div style="font-size:11px;color:var(--text2);line-height:1.8">
      <strong>Q : Comment envoyer un email via Brevo ?</strong><br>
      R : Composer → destinataire → message → "Envoyer via Brevo". Clé API Brevo requise dans Paramètres.<br><br>
      <strong>Q : Comment générer une facture groupée ?</strong><br>
      R : Missions → "Facture mensuelle" → nom exact de l'agence + mois → "Générer".<br><br>
      <strong>Q : Comment voir les emails envoyés à un contact ?</strong><br>
      R : Fiche contact → onglet "Emails & tracking" → historique de tous les emails envoyés via EDLConnect.<br><br>
      <strong>Q : Mes données sont-elles sauvegardées ?</strong><br>
      R : Oui, via Supabase (cloud) + localStorage. Faire des sauvegardes JSON régulières via "💾 Sauvegarder".<br><br>
      <strong>Q : Comment fusionner les doublons ?</strong><br>
      R : Sync Brevo → "Fusionner tous les doublons". Ou Contacts → filtre "Doublons".<br><br>
      <strong>Q : Le CRM fonctionne sur mobile ?</strong><br>
      R : Oui ! Menu ☰ en haut à gauche, navigation rapide en bas de l'écran.<br><br>
      <strong>Q : Comment envoyer le lien extranet à un client ?</strong><br>
      R : Passez le contact en "Client signé ✅" → il reçoit automatiquement un email avec son lien d'accès.<br><br>
      <strong>Q : Le client peut-il voir les commandes des autres agences ?</strong><br>
      R : Non, chaque client ne voit que ses propres commandes — l'isolation est garantie par son email de connexion.<br><br>
      <strong>Q : Comment ajouter un document pour un client dans l'extranet ?</strong><br>
      R : Fiche contact → Modifier → "Documents extranet" → coller le nom + l'URL Google Drive → Enregistrer.<br><br>
      <strong>Q : Comment sauvegarder mes données ?</strong><br>
    ...
    <div class="view" id="view-admin">
      <div class="page-header">
        <div class="page-title">🛡️ Panneau Admin</div>
        <div style="display:flex;gap:8px">
          <button class="btn btn-sm" onclick="loadAdminData()"><i class="ti ti-refresh"></i>Actualiser</button>
          <button class="btn btn-primary btn-sm" onclick="openModal('modal-add-plan')"><i class="ti ti-plus"></i>Ajouter un client</button>
        </div>
      </div>
      <!-- KPIs admin -->
      <div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:14px">
        <div class="kpi"><div class="kpi-label">Total clients</div><div class="kpi-value" id="adm-total">—</div></div>
        <div class="kpi"><div class="kpi-label">Actifs</div><div class="kpi-value" id="adm-active" style="color:var(--green)">—</div></div>
        <div class="kpi"><div class="kpi-label">Plan Pro</div><div class="kpi-value" id="adm-pro" style="color:var(--blue)">—</div></div>
        <div class="kpi"><div class="kpi-label">Plan Gratuit</div><div class="kpi-value" id="adm-free" style="color:var(--amber)">—</div></div>
      </div>
      <!-- KPIs revenus -->
      <div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:18px">
        <div class="kpi" style="border-left:3px solid var(--green)">
          ...
          <div class="kpi-value" id="adm-mrr" style="color:var(--green)">—</div>
          <div style="font-size:10px;color:var(--text2)">revenus mensuels</div>
        </div>
        <div class="kpi" style="border-left:3px solid var(--blue)">
          <div class="kpi-label">📈 ARR estimé</div>
          <div class="kpi-value" id="adm-arr" style="color:var(--blue)">—</div>
          <div style="font-size:10px;color:var(--text2)">revenus annuels</div>
        </div>
        <div class="kpi" style="border-left:3px solid var(--amber)">
          <div class="kpi-label">🔄 Taux conversion</div>
          <div class="kpi-value" id="adm-conversion" style="color:var(--amber)">—</div>
          <div style="font-size:10px;color:var(--text2)">essai → payant</div>
        </div>
        <div class="kpi" style="border-left:3px solid var(--teal)">
          <div class="kpi-label">🆕 Ce mois</div>
          <div class="kpi-value" id="adm-new-month" style="color:var(--teal)">—</div>
          <div style="font-size:10px;color:var(--text2)">nouveaux inscrits</div>
        </div>
      </div>
      <!-- Répartition plans -->
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px">
        <div class="card" style="padding:14px">
          <div style="font-size:12px;font-weight:600;margin-bottom:12px">📊 Répartition des plans</div>
          <div id="adm-plan-chart"></div>
        </div>
        <div class="card" style="padding:14px">
          <div style="font-size:12px;font-weight:600;margin-bottom:12px">📅 Inscriptions ce mois</div>
          <div id="adm-monthly-chart"></div>
        </div>
      </div>
      <!-- Tableau clients -->
      <div class="card">
        <div class="card-head">Comptes clients</div>
        <div style="overflow-x:auto">
          <table class="tbl" style="min-width:700px">
            <thead><tr><th>Email</th><th>Plan</th><th>Statut</th><th>Expiration</th><th>Inscription</th><th>Notes</th><th></th></tr></thead>
    ...
    <div class="view" id="view-reservations">
      <div class="page-header">
        <div class="page-title">📥 Réservations entrantes</div>
        <div style="display:flex;gap:8px">
          <button class="btn btn-sm" onclick="openManualReservationModal()"><i class="ti ti-plus"></i> Saisie manuelle</button>
          <button class="btn btn-primary" onclick="loadReservations()"><i class="ti ti-refresh"></i>Actualiser</button>
        </div>
      </div>

      <!-- KPIs -->
      <div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:18px">
        <div class="kpi"><div class="kpi-label">Total reçues</div><div class="kpi-value" id="resa-total">—</div></div>
        <div class="kpi" style="border-left:3px solid var(--amber)"><div class="kpi-label">⏳ En attente</div><div class="kpi-value" id="resa-pending" style="color:var(--amber)">—</div></div>
        <div class="kpi" style="border-left:3px solid var(--green)"><div class="kpi-label">✅ Confirmées</div><div class="kpi-value" id="resa-confirmed" style="color:var(--green)">—</div></div>
        <div class="kpi" style="border-left:3px solid var(--blue)"><div class="kpi-label">📅 Aujourd'hui</div><div class="kpi-value" id="resa-today" style="color:var(--blue)">—</div></div>
      </div>

      <!-- Statistiques booking -->
      <div id="resa-stats" style="display:none;margin-bottom:18px">
        <div class="card" style="padding:18px">
          <div class="card-head" style="margin-bottom:14px">
            <span>📊 Statistiques</span>
            <select id="resa-stats-period" onchange="renderResaStats()" style="font-size:11px;padding:4px 8px;border-radius:var(--radius);border:1px solid var(--border2)">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
            <div>
              <div style="font-size:11px;font-weight:600;color:var(--text2);margin-bottom:8px">PAR TYPE D'EDL</div>
              <div id="stats-by-type" style="font-size:12px;line-height:2"></div>
            </div>
            <div>
              <div style="font-size:11px;font-weight:600;color:var(--text2);margin-bottom:8px">PAR AGENCE (TOP 5)</div>
              <div id="stats-by-agency" style="font-size:12px;line-height:2"></div>
            </div>
          </div>
          <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border)">
            <div style="font-size:11px;font-weight:600;color:var(--text2);margin-bottom:8px">PAR MOIS</div>
            <div id="stats-by-month" style="font-size:12px;line-height:2;display:flex;gap:12px;flex-wrap:wrap"></div>
          </div>
        </div>
      </div>

      <!-- Notifications push -->
      <div id="resa-notif-banner" style="display:none;background:var(--blue-bg);border-radius:var(--radius);padding:10px 14px;margin-bottom:12px;font-size:12px;color:var(--blue-text);align-items:center;gap:10px">
        <i class="ti ti-bell"></i>
        <span>Activez les notifications pour être alerté des nouvelles réservations même quand l'onglet est en arrière-plan.</span>
        <button class="btn btn-sm" onclick="requestNotifPermission()" style="margin-left:auto;white-space:nowrap">🔔 Activer</button>
        <button onclick="dismissNotifBanner()" style="background:none;border:none;cursor:pointer;color:var(--blue-text);font-size:14px">✕</button>
      </div>

      <div id="resa-loading" style="text-align:center;padding:40px;color:var(--text2);font-size:13px">
        <div style="font-size:32px;margin-bottom:12px">📥</div>
        Clique sur "Actualiser" pour charger les réservations
      </div>

      <div class="card" id="resa-table-wrap" style="display:none">
        <div class="card-head">
          <span id="resa-count">Réservations</span>
          <div style="display:flex;gap:8px">
            <select id="resa-filter" onchange="filterReservations()" style="font-size:11px;padding:4px 8px;border-radius:var(--radius);border:1px solid var(--border2)">
    ...
    <div class="view" id="view-settings">
      ...
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-brand-notion" style="font-size:18px"></i>Notion</div>
        <label>Token d'intégration</label><input type="password" id="set-notion-token" placeholder="ntn_...">
        <label>Page ID</label><input id="set-notion-page" placeholder="xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx">
      </div>
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-mail" style="font-size:18px"></i>Brevo</div>
        <label>Clé API</label><input type="password" id="set-brevo-key" placeholder="xkeysib-...">
      </div>
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-sparkles" style="font-size:18px"></i>IA — Rédaction emails</div>
        <label>Clé API Mistral IA <span style="font-size:10px;color:var(--text2)">(gratuit — modèle mistral-small)</span></label>
        <input type="password" id="set-claude-key" placeholder="...">
        <div style="font-size:11px;color:var(--text2);margin-top:4px">Obtenir une clé gratuite : <a href="https://console.mistral.ai/api-keys" target="_blank" style="color:var(--blue)">console.mistral.ai</a> → "API Keys" → "Create new key"</div>
      </div>
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-users" style="font-size:18px"></i>Agents EDL</div>
        <div style="font-size:11px;color:var(--text2);margin-bottom:10px">Les agents ajoutés ici apparaissent dans le menu déroulant "Agent qui se déplace" lors de la confirmation d'un RDV, et leurs coordonnées sont insérées automatiquement dans la convocation envoyée au locataire.</div>
        <div id="agents-list" style="margin-bottom:12px"></div>
        <div class="form-grid">
          <div><label>Nom complet</label><input id="new-agent-nom" placeholder="Jean Dupont"></div>
          <div><label>Mobile</label><input id="new-agent-tel" placeholder="06 12 34 56 78"></div>
        </div>
        <button class="btn btn-sm" onclick="addAgent()" style="margin-top:8px"><i class="ti ti-user-plus"></i> Ajouter cet agent</button>
      </div>
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-user" style="font-size:18px"></i>Profil</div>
        <label>Nom</label><input id="set-name" value="Thomas Langlade">
        <label>Email</label><input id="set-email" value="contact@edl-idf.com">
        <label>Société</label><input id="set-company" value="EDL IDF">
      </div>
      <div class="settings-section">
        <div class="settings-title"><i class="ti ti-file-invoice" style="font-size:18px"></i>Facturation</div>
        <label>Logo (PNG/JPG, fond transparent recommandé)</label>
        <input type="file" id="set-logo-upload" accept="image/png,image/jpeg" onchange="handleLogoUpload(event)">
        <div id="logo-preview-wrap" style="margin:8px 0;display:none">
          <img id="logo-preview" style="max-height:60px;max-width:200px;border:1px solid var(--border);border-radius:6px;padding:6px;background:#fff">
          <button class="btn btn-sm" onclick="removeLogo()" style="margin-left:8px"><i class="ti ti-trash"></i>Retirer</button>
        </div>
        <label>Adresse du siège</label><input id="set-co-address" placeholder="18 Grande Rue, 91510 Lardy">
        <label>SIRET</label><input id="set-co-siret" placeholder="123 456 789 00012 (14 chiffres)">
        <label>N° TVA intracommunautaire</label><input id="set-co-tva" placeholder="FR12 345678901">
        <label>Capital social</label><input id="set-co-capital" placeholder="Ex : 1 000 €">
        <label>IBAN</label><input id="set-co-iban" placeholder="FR76 XXXX XXXX XXXX XXXX XXXX XXX">
        <label>BIC</label><input id="set-co-bic" placeholder="XXXXXXXX">
        <label>Conditions de paiement (affichées sur la facture)</label>
        <input id="set-co-payterms" placeholder="Ex : Paiement à réception de facture">
        <div style="font-size:11px;color:var(--text2);margin-top:6px">Ces informations apparaissent automatiquement sur chaque facture générée depuis une mission.</div>
      </div>
      <button class="btn btn-primary" onclick="saveSettings()"><i class="ti ti-check"></i>Enregistrer les paramètres</button>
    </div>
    ...
    <div class="view" id="view-booking" style="padding:0;background:#f8f8f6;min-height:100vh">

      <!-- Écran formulaire -->
      <div id="booking-app">

        <!-- Header -->
        <div style="background:#fff;border-bottom:1px solid var(--border);padding:14px 20px;display:flex;align-items:center;gap:10px">
          <div style="background:var(--blue-bg);width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0">
            <span style="font-size:13px;font-weight:700;color:var(--blue);letter-spacing:-1px">ED</span>
          </div>
          <div>
        ...
        <div style="max-width:600px;margin:0 auto;padding:24px 16px 60px">
          ...
          <!-- Hero -->
          <div style="background:var(--blue);border-radius:var(--radius-lg);padding:24px;margin-bottom:24px;color:#fff">
            <div style="font-size:11px;opacity:.75;margin-bottom:3px;text-transform:uppercase;letter-spacing:.05em" id="bk-agency-label">Portail agence</div>
            <div style="font-size:20px;font-weight:700;margin-bottom:6px" id="bk-agency-name">Demande d'état des lieux</div>
            <div style="font-size:12px;opacity:.85;line-height:1.6">Remplissez ce formulaire pour soumettre votre demande. Nous vous confirmons la prise en charge sous 2h.</div>
          </div>

          <!-- Étapes -->
          <div style="display:flex;background:#fff;border-radius:var(--radius);border:1px solid var(--border);overflow:hidden;margin-bottom:20px">
            <div id="bk-step1" style="flex:1;padding:10px 6px;text-align:center;font-size:10px;font-weight:600;color:var(--blue);background:var(--blue-bg);border-right:1px solid var(--border)">
              <div style="font-size:14px;margin-bottom:2px">📋</div>Votre demande
            </div>
            <div id="bk-step2" style="flex:1;padding:10px 6px;text-align:center;font-size:10px;font-weight:600;color:var(--text3);border-right:1px solid var(--border)">
              <div style="font-size:14px;margin-bottom:2px">🏠</div>Le bien
            </div>
            <div id="bk-step3" style="flex:1;padding:10px 6px;text-align:center;font-size:10px;font-weight:600;color:var(--text3)">
              <div style="font-size:14px;margin-bottom:2px">👤</div>Le locataire
            </div>
          </div>

          <!-- Erreur -->
          <div id="bk-error" style="display:none;background:var(--red-bg);color:var(--red-text);border-radius:var(--radius);padding:10px 14px;font-size:12px;margin-bottom:14px"></div>
          ...
          <div id="bk-page1">
            <div class="card" style="margin-bottom:14px">
              ...
              <div style="padding:16px">
                <div class="form-grid">
                  <div><label>Nom de l'agence *</label><input type="text" id="bk-agence" placeholder="Orpi Évry"></div>
                  <div><label>Votre nom *</label><input type="text" id="bk-contact" placeholder="Marie Dupont"></div>
                  <div><label>Email *</label><input type="email" id="bk-email" placeholder="m.dupont@orpi.com"></div>
                  <div><label>Téléphone</label><input type="tel" id="bk-tel" placeholder="06 12 34 56 78"></div>
                </div>
              </div>
            </div>
            <div class="card" style="margin-bottom:14px">
              <div class="card-head"><i class="ti ti-file-check"></i> Type d'état des lieux *</div>
              <div style="padding:16px;display:grid;grid-template-columns:1fr 1fr;gap:8px">
                <button type="button" onclick="bkSelectType('EDL entrant',this)" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit;transition:all .15s">
                  <div style="font-size:18px;margin-bottom:3px">🔑</div>
                  <div style="font-size:12px;font-weight:600">EDL Entrant</div>
                  <div style="font-size:10px;color:var(--text2)">Entrée du locataire</div>
                </button>
                <button type="button" onclick="bkSelectType('EDL sortant',this)" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit;transition:all .15s">
                  <div style="font-size:18px;margin-bottom:3px">🚪</div>
                  <div style="font-size:12px;font-weight:600">EDL Sortant</div>
                  <div style="font-size:10px;color:var(--text2)">Sortie du locataire</div>
                </button>
                <button type="button" onclick="bkSelectType('EDL Sortant / Entrant',this)" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit;transition:all .15s">
                  <div style="font-size:18px;margin-bottom:3px">🔄</div>
                  <div style="font-size:12px;font-weight:600">Sortant + Entrant</div>
                  <div style="font-size:10px;color:var(--text2)">Les deux le même jour</div>
                </button>
                <button type="button" onclick="bkSelectType('Pré-état des lieux',this)" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit;transition:all .15s">
                  <div style="font-size:18px;margin-bottom:3px">🔍</div>
                  <div style="font-size:12px;font-weight:600">Pré-état des lieux</div>
                  <div style="font-size:10px;color:var(--text2)">Avant la sortie</div>
                </button>
              </div>
            </div>
            <button onclick="bkNext(1)" class="btn btn-primary" style="width:100%;justify-content:center;padding:14px;font-size:14px">
              Suivant <i class="ti ti-arrow-right"></i>
            </button>
          </div>

          <!-- PAGE 2 -->
          <div id="bk-page2" style="display:none">
            <div class="card" style="margin-bottom:14px">
              <div class="card-head"><i class="ti ti-home"></i> Adresse du bien</div>
              <div style="padding:16px">
                <label>Adresse complète *</label>
                <input type="text" id="bk-adresse" placeholder="12 rue de la Paix, 91000 Évry" style="margin-bottom:12px">
                <div class="form-grid">
                  <div>
                    <label>Type de bien</label>
                    <select id="bk-bien-type"><option value="">— Choisir —</option><option>Appartement</option><option>Maison</option></select>
                  </div>
                  <div>
                    <label>Typologie</label>
                    <select id="bk-bien-typo"><option value="">— Choisir —</option><option>Studio</option><option>T1</option><option>T2</option><option>T3</option><option>T4</option><option>T5</option><option>T6+</option></select>
                  </div>
                  <div>
                    <label>Meublé / Nu</label>
                    <select id="bk-meuble"><option value="">— Choisir —</option><option>Meublé</option><option>Nu</option></select>
                  </div>
                  <div>
                    <label>Superficie (m²)</label>
                    <input type="number" id="bk-superficie" placeholder="45" min="1">
                  </div>
                  <div>
                    <label>Accès (digicode…)</label>
                    <input type="text" id="bk-acces" placeholder="Code : A1234">
                  </div>
                  <div>
                    <label>Date d'entrée dans le logement</label>
                    <input type="date" id="bk-date-entree">
                  </div>
                </div>
              </div>
            </div>
              ...
              <div style="padding:16px">
                <div class="form-grid">
                  <div>
                    ...
                    <div style="font-size:10px;color:var(--text2);margin-top:4px">Sous réserve de disponibilité</div>
                  </div>
                  <div>
                ...
                <label style="margin-top:10px">Informations complémentaires</label>
                <textarea id="bk-notes" placeholder="Clés à récupérer à l'agence, présence du propriétaire…" style="min-height:70px"></textarea>
              </div>
            </div>
            <div style="display:flex;gap:10px">
              <button onclick="bkPrev(2)" class="btn" style="flex:1;justify-content:center;padding:14px"><i class="ti ti-arrow-left"></i> Retour</button>
              <button onclick="bkNext(2)" class="btn btn-primary" style="flex:2;justify-content:center;padding:14px">Suivant <i class="ti ti-arrow-right"></i></button>
            </div>
          </div>

          <!-- PAGE 3 -->
          <div id="bk-page3" style="display:none">
            <div class="card" style="margin-bottom:14px">
              <div class="card-head"><i class="ti ti-user"></i> Locataire</div>
              <div style="padding:16px">
                <div class="form-grid">
                  <div><label>Nom complet *</label><input type="text" id="bk-loc-nom" placeholder="Jean Martin"></div>
                  <div><label>Téléphone *</label><input type="tel" id="bk-loc-tel" placeholder="06 12 34 56 78"></div>
                </div>
                <label style="margin-top:10px">Email du locataire</label>
                <input type="email" id="bk-loc-email" placeholder="jean.martin@email.fr">
                <div style="font-size:10px;color:var(--text2);margin-top:4px">Il recevra sa convocation une fois le RDV planifié</div>
              </div>
            </div>
            <div id="bk-loc-extra-container"></div>
            <button type="button" onclick="bkAddLocataire()" class="btn btn-sm" style="margin-bottom:14px"><i class="ti ti-user-plus"></i> Ajouter un autre locataire (conjoint, colocataire…)</button>
            <!-- Récap -->
            <div class="card" style="margin-bottom:14px">
              <div class="card-head"><i class="ti ti-clipboard-check"></i> Récapitulatif</div>
              <div id="bk-recap" style="padding:16px;font-size:12px;line-height:2;color:var(--text2)"></div>
            </div>
            <div style="background:var(--blue-bg);border-radius:var(--radius);padding:12px 14px;font-size:11px;color:var(--blue-text);margin-bottom:16px;line-height:1.7">
              <strong>📬 Après envoi :</strong> Vous recevrez un email de confirmation. Thomas vous contactera pour confirmer la date définitive.
            </div>
            <div style="display:flex;gap:10px">
              <button onclick="bkPrev(3)" class="btn" style="flex:1;justify-content:center;padding:14px"><i class="ti ti-arrow-left"></i> Retour</button>
              <button onclick="bkSubmit()" id="bk-submit-btn" class="btn btn-primary" style="flex:2;justify-content:center;padding:14px"><i class="ti ti-send"></i> Envoyer ma demande</button>
            </div>
          </div>
      ...
      <!-- Écran succès -->
      <div id="bk-success" style="display:none;text-align:center;padding:60px 24px;max-width:500px;margin:0 auto">
        <div style="width:72px;height:72px;background:var(--green-bg);border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 20px;font-size:32px">✅</div>
        ...
        <div style="font-size:13px;color:var(--text2);line-height:1.7;margin-bottom:24px">
          Thomas vous contactera sous <strong>2h</strong> pour confirmer la date définitive.
        </div>
        <div style="background:var(--blue-bg);border-radius:var(--radius);padding:16px;text-align:left;font-size:12px;color:var(--blue-text);line-height:2" id="bk-success-recap"></div>
        <div style="margin-top:24px;font-size:12px;color:var(--text2)">
...
<!-- VUE EXTRANET CLIENT -->
<div class="view" id="view-extranet" style="padding:0;background:transparent;min-height:100vh">

  <!-- Header fixe -->
  <div style="background:rgba(13,25,41,0.95);backdrop-filter:blur(10px);padding:16px 20px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(55,138,221,0.2);position:sticky;top:0;z-index:100">
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <div>
        <div style="font-size:13px;font-weight:600;color:#fff">EDL IDF Expert en État des Lieux</div>
        <div style="font-size:10px;color:rgba(255,255,255,0.4)">Espace client</div>
      </div>
    </div>
    <div id="extranet-header-right" style="display:none;align-items:center;gap:8px">
      <a href="tel:0189291429" style="display:flex;align-items:center;gap:5px;font-size:11px;color:rgba(255,255,255,0.5);text-decoration:none;background:rgba(255,255,255,0.06);padding:6px 10px;border-radius:20px;border:0.5px solid rgba(255,255,255,0.1)">
        <i class="ti ti-phone" style="font-size:12px"></i> 01 89 29 14 29
      </a>
      <button onclick="extranetLogout()" style="background:none;border:0.5px solid rgba(255,255,255,0.15);border-radius:20px;padding:6px 10px;cursor:pointer;color:rgba(255,255,255,0.5);font-size:11px;display:flex;align-items:center;gap:4px">
  ...
  <div style="max-width:600px;margin:0 auto;padding:28px 20px">
    ...
    <!-- État 1 : connexion -->
    <div id="extranet-login-card">
      <div style="text-align:center;padding:40px 0 32px">
        <div style="width:64px;height:64px;background:linear-gradient(135deg,#185FA5,#378ADD);border-radius:16px;margin:0 auto 16px;display:flex;align-items:center;justify-content:center;font-size:28px;box-shadow:0 8px 24px rgba(24,95,165,0.4)">🔑</div>
        <div style="font-size:22px;font-weight:700;color:#fff;margin-bottom:8px">Bienvenue</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5)">Connectez-vous pour accéder à votre espace client</div>
      </div>
      <div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;backdrop-filter:blur(10px)">
        <label style="color:rgba(255,255,255,0.7);font-size:12px;font-weight:500;display:block;margin-bottom:8px">Votre adresse email</label>
        <input type="email" id="extranet-email" placeholder="contact@votre-agence.fr"
          style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:10px;color:#fff;padding:12px 14px;width:100%;font-size:13px;margin-bottom:14px;box-sizing:border-box"
          onfocus="this.style.borderColor='#378ADD'" onblur="this.style.borderColor='rgba(255,255,255,0.15)'">
        <button class="btn btn-primary" style="width:100%;justify-content:center;padding:13px;border-radius:10px;font-size:13px;font-weight:600" id="extranet-login-btn" onclick="extranetSendMagicLink()">
          <i class="ti ti-mail"></i> Recevoir mon lien de connexion
        </button>
        <div id="extranet-login-msg" style="font-size:12px;margin-top:12px;text-align:center"></div>
      </div>
      <div style="text-align:center;margin-top:20px;font-size:11px;color:rgba(255,255,255,0.25)">
        Vous recevrez un lien de connexion par email · Aucun mot de passe à retenir
      </div>
    </div>

    <!-- État 2 : tableau de bord client -->
    <div id="extranet-dashboard" style="display:none">

      <!-- Message de bienvenue personnalisé -->
      <div style="margin-bottom:20px" id="extranet-welcome-block">
        <div style="font-size:18px;font-weight:700;color:#fff;margin-bottom:4px">Bonjour 👋</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.5)" id="extranet-user-email"></div>
      </div>

      <!-- KPI cards -->
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:16px">
        <div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:22px;font-weight:700;color:#fff" id="extranet-total">0</div>
          <div style="font-size:10px;color:rgba(255,255,255,0.4);margin-top:2px">Total</div>
        </div>
        <div style="background:rgba(186,117,23,0.15);border:1px solid rgba(186,117,23,0.25);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:22px;font-weight:700;color:#f6c05e" id="extranet-pending">0</div>
          <div style="font-size:10px;color:rgba(246,192,94,0.6);margin-top:2px">En cours</div>
        </div>
        <div style="background:rgba(56,161,105,0.12);border:1px solid rgba(56,161,105,0.25);border-radius:12px;padding:14px;text-align:center">
          <div style="font-size:22px;font-weight:700;color:#68d391" id="extranet-done">0</div>
          <div style="font-size:10px;color:rgba(104,211,145,0.6);margin-top:2px">Réalisées</div>
        </div>
      </div>

      <!-- Bouton nouvelle commande -->
      <button onclick="extranetShowOrderForm()" id="extranet-new-order-btn"
        style="width:100%;background:linear-gradient(135deg,#185FA5,#378ADD);color:#fff;border:none;border-radius:12px;padding:14px;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;margin-bottom:20px;box-shadow:0 4px 16px rgba(24,95,165,0.35)">
        <i class="ti ti-plus" style="font-size:16px"></i> Nouvelle commande d'état des lieux
      </button>

      <!-- Modal détail commande -->
      <div id="extranet-order-detail-modal" style="display:none;position:fixed;inset:0;z-index:10001;background:rgba(0,0,0,0.7);align-items:center;justify-content:center;padding:16px">
        <div style="background:#fff;border-radius:16px;width:100%;max-width:480px;overflow:hidden;box-shadow:0 24px 60px rgba(0,0,0,0.3)">
          <div style="background:linear-gradient(135deg,#185FA5,#378ADD);padding:18px 20px;display:flex;align-items:center;justify-content:space-between">
            <div style="color:#fff;font-size:14px;font-weight:600" id="extranet-detail-title">Détail de la commande</div>
            <button onclick="closeExtranetOrderDetail()" style="background:rgba(255,255,255,0.15);border:none;cursor:pointer;color:#fff;font-size:16px;line-height:1;width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center">×</button>
          </div>
          <div id="extranet-detail-body" style="padding:20px;overflow-y:auto;max-height:70vh"></div>
        </div>
      </div>

      <!-- Liste des commandes -->
      <div style="font-size:11px;font-weight:600;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px">Mes commandes</div>
      <div id="extranet-orders-list"></div>

      <!-- Section statistiques -->
      <div id="extranet-stats" style="display:none;margin-top:8px">
        <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:12px;margin-bottom:12px;overflow:hidden">
          <div style="padding:14px 16px;font-size:12px;font-weight:600;color:rgba(255,255,255,0.7);border-bottom:1px solid rgba(255,255,255,0.08)">📊 Activité — <span id="extranet-stats-year"></span></div>
          <div style="padding:16px">
            <div id="extranet-chart-bars" style="display:flex;align-items:flex-end;gap:6px;height:120px;margin-bottom:8px"></div>
            <div style="display:flex;gap:16px;font-size:10px;color:rgba(255,255,255,0.35);flex-wrap:wrap">
              <span><span style="display:inline-block;width:8px;height:8px;background:#185FA5;border-radius:2px;margin-right:4px"></span>Entrant</span>
              <span><span style="display:inline-block;width:8px;height:8px;background:#E67E22;border-radius:2px;margin-right:4px"></span>Sortant</span>
              <span><span style="display:inline-block;width:8px;height:8px;background:#27AE60;border-radius:2px;margin-right:4px"></span>Autre</span>
            </div>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:12px;overflow:hidden">
            <div style="padding:12px 14px;font-size:11px;font-weight:600;color:rgba(255,255,255,0.5);border-bottom:1px solid rgba(255,255,255,0.08)">Par type</div>
            <div id="extranet-stats-types" style="padding:12px;font-size:12px;color:rgba(255,255,255,0.7)"></div>
          </div>
          <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:12px;overflow:hidden">
            <div style="padding:12px 14px;font-size:11px;font-weight:600;color:rgba(255,255,255,0.5);border-bottom:1px solid rgba(255,255,255,0.08)">Par bien</div>
            <div id="extranet-stats-typo" style="padding:12px;font-size:12px;color:rgba(255,255,255,0.7)"></div>
          </div>
        </div>
      </div>

      <button class="btn btn-sm" id="extranet-stats-toggle" onclick="extranetToggleStats()"
        style="width:100%;justify-content:center;margin-bottom:16px;display:none;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5);border-radius:10px;padding:10px">
        📊 Voir mes statistiques
      </button>

      <!-- Section documents -->
      <div id="extranet-docs-section" style="display:none;margin-bottom:16px">
        <div style="font-size:11px;font-weight:600;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px">Mes documents</div>
        <div id="extranet-docs-list"></div>
      </div>

      <!-- Contact -->
      <div style="background:rgba(24,95,165,0.15);border:1px solid rgba(55,138,221,0.2);border-radius:12px;padding:16px;margin-top:8px;display:flex;align-items:center;gap:14px">
        <div style="width:40px;height:40px;background:#185FA5;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:18px">👨‍💼</div>
        <div style="flex:1">
          <div style="font-size:12px;font-weight:600;color:#7bb8ef">Une question ?</div>
          <div style="font-size:11px;color:rgba(255,255,255,0.4);margin-top:2px">Thomas Langlade · EDL IDF Expert en État des Lieux</div>
        </div>
        <a href="tel:0189291429" style="display:flex;align-items:center;gap:5px;background:#185FA5;color:#fff;text-decoration:none;padding:8px 14px;border-radius:8px;font-size:12px;font-weight:600">
          <i class="ti ti-phone" style="font-size:13px"></i> Appeler
        </a>
      </div>

      <!-- Formulaire de nouvelle commande intégré -->
      <div id="extranet-order-form" style="display:none" class="card" style="margin-bottom:16px">
        <div class="card-head">
          <span><i class="ti ti-clipboard-plus"></i> Nouvelle demande d'état des lieux</span>
          <button onclick="extranetHideOrderForm()" style="background:none;border:none;cursor:pointer;font-size:16px;color:var(--text2)">✕</button>
        </div>
        <div style="padding:16px">

          <div id="extranet-form-error" style="display:none;background:var(--red-bg);color:var(--red-text);border-radius:var(--radius);padding:10px 14px;font-size:12px;margin-bottom:14px"></div>

          <!-- Type EDL -->
          <label style="font-weight:600;margin-bottom:8px;display:block">Type d'état des lieux *</label>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:16px">
            <button type="button" onclick="exSelectType('EDL entrant',this)" class="ex-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit">
              <div style="font-size:13px">🔑 EDL entrant</div><div style="font-size:11px;color:var(--text2)">État des lieux d&#39;entrée</div>
            </button>
            <button type="button" onclick="exSelectType('EDL sortant',this)" class="ex-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit">
              <div style="font-size:13px">🏠 EDL sortant</div><div style="font-size:11px;color:var(--text2)">État des lieux de sortie</div>
            </button>
            <button type="button" onclick="exSelectType('EDL Sortant / Entrant',this)" class="ex-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit">
              <div style="font-size:13px">🔄 EDL Sortant / Entrant</div><div style="font-size:11px;color:var(--text2)">Les deux le même jour</div>
            </button>
            <button type="button" onclick="exSelectType('Pré-état des lieux',this)" class="ex-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:12px;cursor:pointer;background:#fff;text-align:left;font-family:inherit">
              <div style="font-size:13px">📋 Pré-état des lieux</div><div style="font-size:11px;color:var(--text2)">Avant la sortie du locataire</div>
            </button>
          </div>

          <!-- Adresse du bien -->
          <label style="font-weight:600;margin-bottom:8px;display:block">Adresse du bien *</label>
          <input type="text" id="ex-adresse" placeholder="12 rue de la Paix, 91000 Évry" style="margin-bottom:10px">
          <div class="form-grid" style="margin-bottom:10px">
            <div>
              <label>Type de bien</label>
              <select id="ex-bien-type"><option>Appartement</option><option>Maison</option><option>Studio</option><option>Local commercial</option><option>Parking</option><option>Autre</option></select>
            </div>
            <div>
              <label>Meublé / Nu</label>
              <select id="ex-meuble"><option>Nu</option><option>Meublé</option></select>
            </div>
            <div>
              <label>Superficie (m²)</label>
              <input type="number" id="ex-superficie" placeholder="45" min="1">
            </div>
            <div>
              <label>Date d'entrée dans le logement</label>
              <input type="date" id="ex-date-entree">
            </div>
          </div>
          <label>Accès / Digicode</label>
          <input type="text" id="ex-acces" placeholder="Code : A1234" style="margin-bottom:14px">

          <!-- Date souhaitée -->
          <label style="font-weight:600;margin-bottom:8px;display:block">Date souhaitée *</label>
          ...
          <!-- Locataire -->
          <label style="font-weight:600;margin-bottom:8px;display:block">Locataire *</label>
          <div class="form-grid" style="margin-bottom:8px">
            <div><label>Nom complet</label><input type="text" id="ex-loc-nom" placeholder="Jean Martin"></div>
            <div><label>Téléphone</label><input type="tel" id="ex-loc-tel" placeholder="06 12 34 56 78"></div>
          </div>
          <label>Email du locataire</label>
          <input type="email" id="ex-loc-email" placeholder="jean.martin@email.fr" style="margin-bottom:14px">

          <!-- Notes -->
          <label>Informations complémentaires</label>
          <textarea id="ex-notes" placeholder="Clés à récupérer à l'agence, présence du propriétaire..." style="min-height:80px;margin-bottom:16px"></textarea>

          <button class="btn btn-primary" style="width:100%;justify-content:center;padding:14px" id="ex-submit-btn" onclick="extranetSubmitOrder()">
            <i class="ti ti-send"></i> Envoyer ma demande
          </button>
          <div id="ex-success" style="display:none;background:var(--green-bg);color:var(--green-text);border-radius:var(--radius);padding:14px;text-align:center;margin-top:12px;font-size:13px">
            ✅ Votre demande a bien été envoyée ! Nous vous contacterons pour confirmer le rendez-vous.
          </div>
        </div>
      </div>
      ...
      <div id="extranet-stats" style="display:none;margin-top:8px">
        <div class="card" style="margin-bottom:12px">
          <div class="card-head">📊 Activité mensuelle — <span id="extranet-stats-year"></span></div>
          <div style="padding:16px">
            <div id="extranet-chart-bars" style="display:flex;align-items:flex-end;gap:6px;height:120px;margin-bottom:8px"></div>
            <div style="display:flex;gap:16px;font-size:11px;color:var(--text2);flex-wrap:wrap">
              <span><span style="display:inline-block;width:10px;height:10px;background:#185FA5;border-radius:2px;margin-right:4px"></span>EDL entrant</span>
              <span><span style="display:inline-block;width:10px;height:10px;background:#E67E22;border-radius:2px;margin-right:4px"></span>EDL sortant</span>
              <span><span style="display:inline-block;width:10px;height:10px;background:#27AE60;border-radius:2px;margin-right:4px"></span>Pré-EDL / Autre</span>
            </div>
          </div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px">
          <div class="card">
            <div class="card-head">🔑 Par type d'EDL</div>
            <div id="extranet-stats-types" style="padding:12px;font-size:12px"></div>
          </div>
          <div class="card">
            <div class="card-head">🏠 Par typologie de bien</div>
            <div id="extranet-stats-typo" style="padding:12px;font-size:12px"></div>
          </div>
        </div>
      </div>

      <button class="btn btn-sm" id="extranet-stats-toggle" onclick="extranetToggleStats()" style="width:100%;justify-content:center;margin-bottom:16px;display:none">
        📊 Voir mes statistiques
      </button>
      ...
      <div id="extranet-docs-section" style="display:none;margin-bottom:16px">
        <div class="card">
          <div class="card-head">📁 Mes documents</div>
          <div id="extranet-docs-list" style="padding:12px"></div>
        </div>
      </div>
    </div>
...
<!-- MODAL DETAIL EVENEMENT AGENDA -->
<div class="modal-bg" id="modal-evt-detail">
  <div class="modal" style="width:480px">
    <div class="modal-title" id="evt-detail-title">
      Détail
      <button class="modal-close" onclick="closeModal('modal-evt-detail')"><i class="ti ti-x"></i></button>
    </div>
    <div style="padding:16px">
      <div id="evt-detail-body" style="margin-bottom:14px"></div>
      <label style="font-weight:600;display:block;margin-bottom:6px">📝 Notes</label>
      <textarea id="evt-detail-notes" style="min-height:90px;margin-bottom:12px" placeholder="Ajouter une note sur ce rendez-vous…"></textarea>
      <div style="display:flex;gap:8px;justify-content:flex-end">
        <button class="btn" onclick="closeModal('modal-evt-detail')">Fermer</button>
        <button class="btn btn-primary" id="evt-detail-mission-btn" style="display:none" onclick="goToEvtMission()"><i class="ti ti-arrow-right"></i> Voir la mission</button>
        <button class="btn btn-primary" onclick="saveEvtNotes()"><i class="ti ti-check"></i> Enregistrer les notes</button>
      </div>
    </div>
  </div>
</div>
...
<!-- MODAL CONFIRM RDV -->
<div class="modal-bg" id="modal-confirm-rdv">
  <div class="modal" style="width:500px">
    <div class="modal-title">
      📅 Confirmer le RDV
      <button class="modal-close" onclick="closeModal('modal-confirm-rdv')"><i class="ti ti-x"></i></button>
    </div>
    <div style="background:var(--blue-bg);border-radius:var(--radius);padding:14px;margin-bottom:16px;font-size:12px;color:var(--blue-text);line-height:1.8" id="confirm-rdv-recap"></div>
    <div style="background:var(--amber-bg);border-radius:var(--radius);padding:12px 14px;margin-bottom:16px;font-size:12px;color:var(--amber-text)">
      ⚡ <strong>Les emails seront envoyés immédiatement</strong> à l'agent et au locataire avec les informations ci-dessus.
    </div>
    <div class="form-grid" style="margin-bottom:4px">
      <div>
        <label>📅 Date définitive *</label>
        <input type="date" id="confirm-rdv-date">
      </div>
      <div>
        <label>🕐 Heure définitive *</label>
        <select id="confirm-rdv-heure">
          <option value="">— Choisir —</option>
          <option>08h00</option><option>08h30</option><option>09h00</option><option>09h30</option>
          <option>10h00</option><option>10h30</option><option>11h00</option><option>11h30</option>
          <option>12h00</option><option>13h00</option><option>13h30</option><option>14h00</option>
          <option>14h30</option><option>15h00</option><option>15h30</option><option>16h00</option>
          <option>16h30</option><option>17h00</option><option>17h30</option><option>18h00</option>
          <option>18h30</option><option>19h00</option><option>19h30</option>
        </select>
      </div>
    </div>
    <label>Email de l'agent <span style="color:var(--red)">*</span></label>
    <input type="email" id="confirm-rdv-agent-email" placeholder="agent@agence.fr" style="margin-bottom:10px">
    <label>👤 Expert EDL qui se déplace</label>
    <select id="confirm-rdv-expert" style="margin-bottom:10px">
      <option value="">— Non précisé —</option>
    </select>
    <div class="form-grid" style="margin-bottom:4px">
      <div>
        <label>Civilité du locataire</label>
        <select id="confirm-rdv-civilite">
          <option value="">— Choisir —</option>
      ...
      <div>
        ...
        <input type="text" id="confirm-rdv-loc-nom" placeholder="Jean Martin">
      </div>
    </div>
    <label>Email du locataire</label>
    <input type="email" id="confirm-rdv-loc-email" placeholder="locataire@email.fr" style="margin-bottom:10px">
    <label>Message personnalisé (optionnel)</label>
    <textarea id="confirm-rdv-message" style="min-height:70px" placeholder="Ex: Merci de prévoir les clés et les relevés de compteurs..."></textarea>
    <div style="margin-top:16px;display:flex;gap:8px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-confirm-rdv')">Annuler</button>
      <button class="btn btn-primary" id="confirm-rdv-btn" onclick="sendConfirmRdv()">
...
<!-- MODAL SAISIE MANUELLE RESERVATION -->
<div class="modal-bg" id="modal-manual-resa">
  <div class="modal" style="width:540px">
    <div class="modal-title">
      ✏️ Nouvelle réservation manuelle
      <button class="modal-close" onclick="closeModal('modal-manual-resa')"><i class="ti ti-x"></i></button>
    </div>
    <div style="padding:16px;max-height:70vh;overflow-y:auto">
      <div id="manual-resa-error" style="display:none;background:var(--red-bg);color:var(--red-text);border-radius:var(--radius);padding:10px 14px;font-size:12px;margin-bottom:12px"></div>

      <div class="form-grid" style="margin-bottom:10px">
        <div><label>Agence <span style="color:var(--red)">*</span></label><input type="text" id="mr-agence" placeholder="Nom de l'agence"></div>
        <div><label>Email agence <span style="color:var(--red)">*</span></label><input type="email" id="mr-email" placeholder="contact@agence.fr"></div>
      </div>
      <div class="form-grid" style="margin-bottom:10px">
        <div><label>Contact</label><input type="text" id="mr-contact" placeholder="Prénom Nom"></div>
        <div><label>Tél agence</label><input type="tel" id="mr-tel" placeholder="01 23 45 67 89"></div>
      </div>

      <label style="font-weight:600;display:block;margin-bottom:8px">Type d'EDL <span style="color:var(--red)">*</span></label>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:12px">
        <button type="button" onclick="mrSelectType('EDL entrant',this)" class="mr-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:8px;cursor:pointer;background:var(--bg);font-family:inherit;font-size:12px">🔑 EDL entrant</button>
        <button type="button" onclick="mrSelectType('EDL sortant',this)" class="mr-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:8px;cursor:pointer;background:var(--bg);font-family:inherit;font-size:12px">🏠 EDL sortant</button>
        <button type="button" onclick="mrSelectType('EDL Sortant / Entrant',this)" class="mr-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:8px;cursor:pointer;background:var(--bg);font-family:inherit;font-size:12px">🔄 Sortant / Entrant</button>
        <button type="button" onclick="mrSelectType('Pré-état des lieux',this)" class="mr-type-btn" style="border:1.5px solid var(--border);border-radius:var(--radius);padding:8px;cursor:pointer;background:var(--bg);font-family:inherit;font-size:12px">📋 Pré-état des lieux</button>
      </div>

      <label>Adresse du bien <span style="color:var(--red)">*</span></label>
      <input type="text" id="mr-adresse" placeholder="12 rue de la Paix, 91000 Évry" style="margin-bottom:10px">

      <div class="form-grid" style="margin-bottom:10px">
        <div><label>Type de bien</label><select id="mr-bien-type"><option>Appartement</option><option>Maison</option><option>Studio</option><option>Local commercial</option><option>Parking</option><option>Autre</option></select></div>
        <div><label>Meublé / Nu</label><select id="mr-meuble"><option>Nu</option><option>Meublé</option></select></div>
        <div><label>Superficie (m²)</label><input type="number" id="mr-superficie" placeholder="45" min="1"></div>
        <div><label>Date d'entrée</label><input type="date" id="mr-date-entree"></div>
      </div>

      <label>Accès / Digicode</label>
      <input type="text" id="mr-acces" placeholder="Code : A1234" style="margin-bottom:10px">

      <div class="form-grid" style="margin-bottom:10px">
        <div><label>Date souhaitée <span style="color:var(--red)">*</span></label><input type="date" id="mr-date"></div>
        <div><label>Heure souhaitée</label><select id="mr-heure"><option value="">— Flexible —</option><option>08:00</option><option>09:00</option><option>10:00</option><option>11:00</option><option>12:00</option><option>13:00</option><option>14:00</option><option>15:00</option><option>16:00</option><option>17:00</option><option>18:00</option></select></div>
      </div>

      <label style="font-weight:600;display:block;margin-bottom:6px">Locataire <span style="color:var(--red)">*</span></label>
      <div class="form-grid" style="margin-bottom:8px">
        <div><label>Nom complet</label><input type="text" id="mr-loc-nom" placeholder="Jean Martin"></div>
        <div><label>Téléphone</label><input type="tel" id="mr-loc-tel" placeholder="06 12 34 56 78"></div>
      </div>
      <label>Email du locataire</label>
      <input type="email" id="mr-loc-email" placeholder="jean.martin@email.fr" style="margin-bottom:10px">

      <label>Notes</label>
      <textarea id="mr-notes" placeholder="Informations complémentaires..." style="min-height:60px;margin-bottom:16px"></textarea>

      <div style="display:flex;gap:8px;justify-content:flex-end">
        <button class="btn" onclick="closeModal('modal-manual-resa')">Annuler</button>
        <button class="btn btn-primary" id="mr-submit-btn" onclick="submitManualReservation()"><i class="ti ti-send"></i> Créer et envoyer les emails</button>
      </div>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="modal-contact">
  <div class="modal">
    <div class="modal-title">Nouveau contact <button class="modal-close" onclick="closeModal('modal-contact')"><i class="ti ti-x"></i></button></div>
    <div class="form-grid">
      <div><label>Entreprise *</label><input id="c-ent" placeholder="Orpi Évry"></div>
      <div><label>Contact</label><input id="c-contact" placeholder="Marie Dupont"></div>
      <div><label>Email</label><input type="email" id="c-email" placeholder="m.dupont@orpi.com"></div>
      <div><label>Téléphone</label><input id="c-tel" placeholder="06 12 34 56 78"></div>
    </div>
    <label>Statut</label><select id="c-statut"><option>Cible potentielle</option><option>Client actif</option><option>Client signé ✅</option><option>Partenaire</option><option>Inactif</option></select>
    <label>Source</label><select id="c-source"><option>Démarchage</option><option>Recommandation</option><option>Relation</option><option>Site web</option><option>Brevo</option></select>
    <label>Notes</label><textarea id="c-notes" style="min-height:60px"></textarea>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-contact')">Annuler</button>
      <button class="btn btn-primary" onclick="saveContact()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>

<div class="modal-bg" id="modal-deal">
  <div class="modal">
    <div class="modal-title">Nouvelle opportunité <button class="modal-close" onclick="closeModal('modal-deal')"><i class="ti ti-x"></i></button></div>
    <label>Agence *</label><input id="d-agence" placeholder="Foncia Longjumeau">
    <label>Montant estimé (€/mois)</label><input type="number" id="d-montant" placeholder="800">
    <label>Étape</label><select id="d-etape"><option>Prospect</option><option>Qualifié</option><option>Proposé</option><option>Négociation</option><option>Gagné</option></select>
    <label>Probabilité (%)</label><input type="number" id="d-proba" placeholder="50">
    <label>Notes</label><textarea id="d-notes" style="min-height:60px"></textarea>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-deal')">Annuler</button>
      <button class="btn btn-primary" onclick="saveDeal()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="modal-mission">
  <div class="modal">
    <div class="modal-title"><span id="modal-mission-title">Nouvelle mission EDL</span> <button class="modal-close" onclick="closeModal('modal-mission')"><i class="ti ti-x"></i></button></div>
    <div class="form-grid">
      <div><label>Agence / Client *</label><input id="m-agence" placeholder="Century 21 Évry" oninput="debounceAC(autocompleteMission,this.value)">
        <div id="m-agence-suggest" style="display:none;position:absolute;background:var(--bg);border:1px solid var(--border2);border-radius:var(--radius);z-index:50;max-height:160px;overflow-y:auto;box-shadow:0 4px 12px rgba(0,0,0,.1);min-width:200px"></div>
      </div>
      <div><label>Type client</label><select id="m-type-client"><option>Professionnel</option><option>Particulier</option></select></div>
      <div><label>Email client</label><input type="email" id="m-email" placeholder="contact@agence.fr"></div>
      <div><label>Type EDL</label><select id="m-type"><option>EDL entrant</option><option>EDL sortant</option><option>EDL Sortant / Entrant</option><option>Pré-état des lieux</option></select></div>
      <div><label>Adresse du bien</label><input id="m-adresse" placeholder="12 rue de la Paix, 91000 Évry"></div>
      <div><label>Date & heure</label><input type="datetime-local" id="m-date"></div>
      <div><label>Type de bien</label><select id="m-bien-type"><option value="">—</option><option>Maison</option><option>Appartement</option></select></div>
      <div><label>Typologie</label><select id="m-bien-typo"><option value="">—</option><option>T1</option><option>T2</option><option>T3</option><option>T4</option><option>T5</option><option>T6</option><option>T7</option></select></div>
      <div><label>Meublé / Nu</label><select id="m-bien-meuble"><option value="">—</option><option>Meublé</option><option>Nu</option></select></div>
      <div><label>Montant (€)</label><input type="number" id="m-montant" placeholder="160"></div>
      <div><label>Statut</label><select id="m-statut"><option>planifiée</option><option>en cours</option><option>terminée</option><option>facturée</option></select></div>
    </div>
    <label>Notes</label><textarea id="m-notes" style="min-height:50px"></textarea>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-mission')">Annuler</button>
      <button class="btn btn-primary" id="mission-save-btn" onclick="saveMission()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>

<div class="modal-bg" id="modal-invoice-monthly">
  <div class="modal">
    <div class="modal-title">Facture mensuelle groupée <button class="modal-close" onclick="closeModal('modal-invoice-monthly')"><i class="ti ti-x"></i></button></div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:10px">Regroupe toutes les missions <strong>non encore facturées</strong> de ce client sur le mois choisi, dans une seule facture.</div>
    <label>Client (nom exact de l'agence)</label>
    <input id="inv-month-client" placeholder="Immo Gestion Era">
    <label>Mois</label>
    <input type="month" id="inv-month-month">
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-invoice-monthly')">Annuler</button>
      <button class="btn btn-primary" onclick="confirmMonthlyInvoice()"><i class="ti ti-file-invoice"></i>Générer la facture</button>
    </div>
  </div>
</div>

<div class="modal-bg" id="modal-invoice-particulier">
  <div class="modal">
    <div class="modal-title">Facture pour un particulier <button class="modal-close" onclick="closeModal('modal-invoice-particulier')"><i class="ti ti-x"></i></button></div>
    <div style="font-size:12px;color:var(--text2);margin-bottom:10px">Pour une prestation hors agence (vente directe à un particulier), sans créer de mission.</div>
    <label>Nom du client *</label>
    <input id="inv-p-nom" placeholder="M. et Mme Dupont">
    <label>Email (optionnel)</label>
    <input type="email" id="inv-p-email" placeholder="dupont@email.fr">
    <label>Adresse</label>
    <input id="inv-p-adresse" placeholder="12 rue de la Paix, 91000 Évry">
    <label>Désignation de la prestation</label>
    <input id="inv-p-designation" placeholder="État des lieux">
    <div class="form-grid">
      <div><label>Date de la prestation</label><input type="date" id="inv-p-date"></div>
      <div><label>Montant HT (€) *</label><input type="number" id="inv-p-montant" placeholder="160"></div>
    </div>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-invoice-particulier')">Annuler</button>
      <button class="btn btn-primary" onclick="confirmParticulierInvoice()"><i class="ti ti-file-invoice"></i>Générer la facture</button>
    </div>
  </div>
</div>

<div class="modal-bg" id="modal-rdv">
  <div class="modal">
    <div class="modal-title">Nouveau RDV <button class="modal-close" onclick="closeModal('modal-rdv')"><i class="ti ti-x"></i></button></div>
    <label>Titre *</label><input id="r-titre" placeholder="Appel découverte — Orpi Évry">
    <div class="form-grid">
      <div><label>Date & heure</label><input type="datetime-local" id="r-date"></div>
      <div><label>Durée</label><select id="r-duree"><option>30 min</option><option>1h</option><option>1h30</option><option>2h</option></select></div>
    </div>
    <label>Type</label><select id="r-type"><option>Appel commercial</option><option>Visio</option><option>Rendez-vous physique</option><option>Mission EDL</option></select>
    <label>Contact / Agence</label><input id="r-contact" placeholder="Julie Moreau — Foncia">
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-rdv')">Annuler</button>
      <button class="btn btn-primary" onclick="saveRdv()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>

<!-- MODAL PROSPECT PROSPECTION -->
<div class="modal-bg" id="modal-prosp">
  <div class="modal">
    <div class="modal-title">Ajouter un prospect <button class="modal-close" onclick="closeModal('modal-prosp')"><i class="ti ti-x"></i></button></div>
    <label>Agence / Entreprise *</label><input id="pp-agence" placeholder="Century 21 Évry">
    <div class="form-grid">
      <div><label>Contact</label><input id="pp-contact" placeholder="Marie Dupont"></div>
      <div><label>Email</label><input type="email" id="pp-email" placeholder="m.dupont@century21.fr"></div>
    </div>
    <div class="form-grid">
      <div><label>Téléphone</label><input id="pp-tel" placeholder="+33 1 23 45 67 89"></div>
      <div><label>Département</label><input id="pp-dept" placeholder="91"></div>
    </div>
    <label>Étape</label>
    <select id="pp-etape">
      <option>À contacter</option>
      <option>Email envoyé</option>
      <option>Email ouvert</option>
    ...
    <div class="form-grid">
      <div><label>CA estimé (€/mois)</label><input type="number" id="pp-ca" placeholder="ex: 800"></div>
      <div><label>Notes</label><input id="pp-notes-short" placeholder="Remarques…"></div>
    </div>
    <label>Notes détaillées</label><textarea id="pp-notes" style="min-height:50px" placeholder="Historique, infos importantes…"></textarea>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-prosp')">Annuler</button>
      <button class="btn btn-primary" onclick="saveProspect()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>
...
<div class="modal-bg" id="modal-fiche">
  <div class="modal" style="width:600px;max-width:96vw">
    <div class="modal-title">
      <div class="flex-row" id="fiche-title-row"><div class="avatar-lg" id="fiche-avatar"></div><div><div style="font-size:16px;font-weight:700" id="fiche-name"></div><div style="font-size:12px;color:var(--text2)" id="fiche-sub"></div></div></div>
      <button class="modal-close" onclick="closeModal('modal-fiche')"><i class="ti ti-x"></i></button>
    </div>
    <div style="display:flex;gap:0;border-bottom:1px solid var(--border);margin-bottom:14px">
      <button class="tab active" id="ftab-infos" onclick="ficheTab('infos',this)">Informations</button>
      <button class="tab" id="ftab-commandes" onclick="ficheTab('commandes',this)">Commandes <span id="ftab-commandes-count"></span></button>
      <button class="tab" id="ftab-taches" onclick="ficheTab('taches',this)">Tâches <span id="ftab-taches-count" style="font-size:10px;background:var(--blue-bg);color:var(--blue-text);padding:1px 5px;border-radius:8px;margin-left:2px"></span></button>
      <button class="tab" id="ftab-emails" onclick="ficheTab('emails',this)">Emails & tracking</button>
      <button class="tab" id="ftab-edit" onclick="ficheTab('edit',this)">Modifier</button>
    </div>
    <div id="fiche-infos">
      <div class="fiche-grid" id="fiche-fields"></div>
      <div style="margin-top:6px;padding:10px;background:var(--bg2);border-radius:var(--radius);font-size:12px" id="fiche-notes-display"></div>
      <div style="display:flex;gap:7px;margin-top:12px;flex-wrap:wrap">
        <button class="btn btn-primary btn-sm" id="fiche-email-btn"><i class="ti ti-send"></i>Envoyer un email</button>
        <button class="btn btn-sm" onclick="ficheTab('edit',document.getElementById('ftab-edit'))"><i class="ti ti-edit"></i>Modifier</button>
        <button class="btn btn-sm" id="fiche-booking-btn" onclick="copyBookingLink()" style="background:var(--green-bg);color:var(--green-text);border-color:var(--green)"><i class="ti ti-link"></i>Lien booking</button>
      </div>
    </div>
    <div id="fiche-commandes" style="display:none">
      <div id="fiche-commandes-list"></div>
      <div style="margin-top:10px">
        <button class="btn btn-primary btn-sm" onclick="addMissionToFiche()"><i class="ti ti-plus"></i>Ajouter une mission manuellement</button>
      </div>
    </div>

    <div id="fiche-taches" style="display:none">
      <!-- Formulaire ajout tâche -->
      <div class="card" style="margin-bottom:12px;padding:12px">
        <div style="font-size:12px;font-weight:600;margin-bottom:8px;color:var(--text2)">Nouvelle tâche</div>
        <input type="text" id="new-task-titre" placeholder="Titre de la tâche (ex: Rappeler ce client)" style="margin-bottom:8px">
        <div class="form-grid" style="margin-bottom:8px">
          <div><label>Date d'échéance</label><input type="date" id="new-task-date"></div>
          <div><label>Heure</label><input type="time" id="new-task-heure" placeholder="09:00"></div>
        </div>
        <textarea id="new-task-notes" placeholder="Notes optionnelles…" style="min-height:50px;margin-bottom:8px"></textarea>
        <button class="btn btn-primary btn-sm" onclick="addContactTask()"><i class="ti ti-plus"></i> Ajouter la tâche</button>
      </div>
      <!-- Liste des tâches -->
      <div id="fiche-taches-list"></div>
    </div>
    <div id="fiche-emails" style="display:none">
      <div id="fiche-emails-list"></div>
    </div>
    <div id="fiche-edit" style="display:none">
      <div class="form-grid">
        <div><label>Entreprise</label><input id="fe-ent"></div>
        <div><label>Contact</label><input id="fe-contact"></div>
        <div><label>Email</label><input type="email" id="fe-email"></div>
        <div><label>Téléphone</label><input id="fe-tel"></div>
      </div>
      <div class="form-grid">
        <div>
          <label>Statut</label>
          <select id="fe-statut" onchange="onContactStatusChange(this)"><option>Cible potentielle</option><option>Client actif</option><option>Client signé ✅</option><option>Partenaire</option><option>Inactif</option></select>
        </div>
        <div>
          <label>Type de client</label>
          <select id="fe-type-client"><option>Professionnel</option><option>Particulier</option></select>
        </div>
        <div>
          <label>Source</label>
          <select id="fe-source"><option>Démarchage</option><option>Recommandation</option><option>Relation</option><option>Site web</option><option>Brevo</option><option>Excel</option><option>Cal.com</option></select>
        </div>
        <div>
          <label>Dernier contact</label>
          <input type="date" id="fe-last-contact">
        </div>
      </div>
      <label>Moyen de dernier contact</label>
      <select id="fe-moyen-contact">
      ...
      <label>Notes</label>
      <textarea id="fe-notes" style="min-height:70px" placeholder="Remarques, historique, infos importantes…"></textarea>

      <label style="margin-top:14px;font-weight:600;display:block">📁 Documents extranet (liens Google Drive)</label>
      <div style="font-size:11px;color:var(--text2);margin-bottom:8px">Ces documents seront visibles par le client dans son espace extranet. Collez l'URL de partage Google Drive de chaque document.</div>
      <div id="fe-docs-list"></div>
      <button type="button" onclick="feAddDoc()" class="btn btn-sm" style="margin-top:6px"><i class="ti ti-plus"></i> Ajouter un document</button>

      <div style="margin-top:12px;display:flex;gap:7px;justify-content:flex-end">
        <button class="btn btn-danger btn-sm" onclick="deleteContact()"><i class="ti ti-trash"></i>Supprimer</button>
        <button class="btn btn-primary" onclick="saveContactEdit()"><i class="ti ti-check"></i>Enregistrer</button>
      </div>
    </div>
  </div>
</div>
...
<!-- MODAL ADMIN PLAN -->
<div class="modal-bg" id="modal-add-plan">
  <div class="modal">
    <div class="modal-title">Gérer le plan client <button class="modal-close" onclick="closeModal('modal-add-plan')"><i class="ti ti-x"></i></button></div>
    <label>Email du client</label><input id="adm-email" placeholder="client@email.com">
    <label>Plan</label>
    <select id="adm-plan">
      <option value="free">Gratuit (max 100 contacts, 20 missions)</option>
      <option value="starter">Starter (max 500 contacts, 100 missions)</option>
      <option value="pro">Pro (illimité)</option>
    </select>
    <label>Statut</label>
    <select id="adm-status">
      <option value="active">Actif</option>
      <option value="signed">Client signé ✅</option>
      <option value="suspended">Suspendu</option>
      <option value="expired">Expiré</option>
    </select>
    <label>Date d'expiration (optionnel)</label>
    <input type="date" id="adm-expires">
    <label>Notes internes</label>
    <textarea id="adm-notes" style="min-height:60px" placeholder="Ex: Paiement reçu le 01/06/2026..."></textarea>
    <div style="margin-top:14px;display:flex;gap:7px;justify-content:flex-end">
      <button class="btn" onclick="closeModal('modal-add-plan')">Annuler</button>
      <button class="btn btn-primary" onclick="saveAdminPlan()"><i class="ti ti-check"></i>Enregistrer</button>
    </div>
  </div>
</div>
...
const PROSP_STAGES=[
  ...
  {key:'email_envoye',label:'Email envoyé',color:'#185FA5',bg:'#E6F1FB'},
...
function renderProspection(){
  ...
  stats.innerHTML=`
    ...
    <div style="background:var(--bg2);border:1px solid var(--border);border-radius:var(--radius);padding:6px 12px;font-size:12px"><span style="font-weight:600;font-size:16px;color:#185FA5">${actifs}</span> en cours</div>
...
function renderCAPanel(){
  ...
  document.getElementById('ca-mensuel').innerHTML=`${caMensuelHT.toLocaleString('fr-FR')} € <span style="font-size:12px;color:#888">HT</span><div style="font-size:13px;color:#185FA5;margin-top:2px">${fmtTTC(caMensuelHT)} TTC</div>`;
  document.getElementById('ca-trim').innerHTML=`${caTriHT.toLocaleString('fr-FR')} € <span style="font-size:12px;color:#888">HT</span><div style="font-size:13px;color:#185FA5;margin-top:2px">${fmtTTC(caTriHT)} TTC</div>`;
  document.getElementById('ca-annuel').innerHTML=`${caAnnuelHT.toLocaleString('fr-FR')} € <span style="font-size:12px;color:#888">HT</span><div style="font-size:13px;color:#185FA5;margin-top:2px">${fmtTTC(caAnnuelHT)} TTC</div>`;
  ...
  document.getElementById('ca-tbody').innerHTML=sorted.length?sorted.map(p=>`<tr>
    ...
    <td style="font-size:11px;color:#185FA5">${fmtTTC(p.ca)}</td>
    <td style="font-size:12px">${((p.ca||0)*3).toLocaleString('fr-FR')} € HT</td>
    <td style="font-size:11px;color:#185FA5">${fmtTTC((p.ca||0)*3)}</td>
...
function updateObjectifs(){
  ...
  const pctBar=(val,obj,label)=>{
    ...
    const color=pct>=100?'#3B6D11':pct>=70?'#185FA5':pct>=40?'#854F0B':'#A32D2D';
...
function notify(msg,type=''){
  ...
  n.textContent=msg;
  n.style.background=type==='warn'?'#854F0B':type==='err'?'#A32D2D':'#185FA5';
...
function detectDuplicates(){
  ...
  DB.contacts.forEach((c,i)=>{
    ...
    // Doublon UNIQUEMENT si même email (non vide)
    if(email){
      if(emailMap[email]!==undefined){
        dups.push({i1:emailMap[email],i2:i,type:'email',val:c.email});
      } else {
        emailMap[email]=i;
      }
    }
...
async function loadGmailEmails(contactId, email){
  ...
  try{
    const resp=await fetch(`/api/gmail-emails?email=${encodeURIComponent(email)}`);
    if(!resp.ok){
      ...
      <div style="padding:10px 12px;background:var(--bg2);display:flex;justify-content:space-between;align-items:flex-start">
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
            ...
            <div style="font-weight:600;font-size:12px">${t.objet||'—'}</div>
          </div>
...
function renderContacts(){
  ...
  document.getElementById('contacts-tbody').innerHTML=list.length?list.map(c=>{
    ...
    return `<tr class="clickable ${dup?'dup-row':''}" onclick="openFiche('${c.id}')">
      <td><div class="flex-row"><div class="avatar" style="font-size:9px;${dup?'background:var(--amber-bg);color:var(--amber-text)':''}">${initials(c.entreprise)}</div><span style="font-size:11px;font-weight:500">${c.entreprise}</span>${dup?'<span class="badge b-amber" style="font-size:9px">doublon</span>':''}</div></td>
      <td style="font-size:11px">${c.contact||'—'}</td>
      <td><a href="mailto:${c.email}" style="color:var(--blue);text-decoration:none;font-size:11px" onclick="event.stopPropagation()">${c.email||'—'}</a></td>
      <td style="font-size:11px">${c.tel||'<span style="color:var(--red);font-size:10px">—</span>'}</td>
      ...
      <td style="font-size:10px;color:var(--text2)">${c.lastContact?`<strong style="color:var(--text)">${fmtDate(c.lastContact)}</strong>`:'—'}</td>
      <td style="font-size:10px">${c.moyenContact||'—'}</td>
    ...
    return `<div class="pipe-col" ondragover="event.preventDefault();this.style.background='var(--blue-bg)'" ondragleave="this.style.background=''" ondrop="dropDeal(event,'${stage}');this.style.background=''">
      ...
      ${cards.map(d=>{
        return `<div class="pipe-card" draggable="true" ondragstart="dragDeal(event,'${d.id}')" style="cursor:grab">
          ...
          <div class="pipe-card-amount">${(d.montant||0).toLocaleString('fr-FR')} €/m</div>
          <div class="pipe-card-meta">${d.proba}% · ${d.notes||'—'}</div>
        </div>`;
      }).join('')}
      ${total?`<div style="font-size:10px;color:var(--text2);text-align:right;margin-top:5px">${total.toLocaleString('fr-FR')} €/m</div>`:''}</div>`;
  }).join('');
}
...
function renderTracking(){
  const statColor={'Envoyé':'#888','Ouvert':'#3B6D11','Cliqué':'#185FA5','Répondu':'#854F0B','Sans suite':'#A32D2D'};
  ...
  toast.innerHTML=`
    <div style="flex:1">
      ...
      <div style="margin-top:6px;height:3px;background:#333;border-radius:2px;overflow:hidden">
        <div id="toast-bar" style="height:100%;width:100%;background:#185FA5;border-radius:2px;transition:width 1s linear"></div>
      </div>
    </div>
...
async function silentSyncBrevo(){
  ...
  try{
    let resp;
    try{
      resp=await fetch('/api/brevo-contacts?t='+Date.now());
    }catch(fetchErr){
  ...
  if(UI.vue==='semaine'){
    ...
    body.innerHTML=days.map(d=>{
      ...
      return `<div class="cal-day${isT?' today':''}" onclick="calDayClick('${fd}',event)" style="min-height:100px">
        ${evts.map(e=>`<div class="cal-ev ${e.evtType==='mission'?'ev-green':e.evtType==='task'?'ev-amber':'ev-blue'}" style="font-size:10px;margin-bottom:2px;cursor:pointer" onclick="event.stopPropagation();${e.evtType==='task'?`openTaskFromCalendar('${e.contactId}','${e.id}')`:`openEvtDetail('${e.evtType}','${e.id||e._supaId||''}')`}">
          ${fmtDT(e.date).split(' ')[1]||''} ${(e.titre||e.agence||'').substring(0,20)}
        </div>`).join('')}
    ...
    document.getElementById('rdv-list').innerHTML=weekEvts.length?weekEvts.map(e=>{
      const gcalUrl = e.evtType==='rdv'
        ...
        : googleCalLink(`EDL IDF — ${e.type||'EDL'} · ${e.agence}`, e.date, '2h', e.adresse, `Agence: ${e.agence}\nMontant: ${e.montant}€ HT`);
      return `<div style="border-left:3px solid ${e.evtType==='mission'?'#3B6D11':'#185FA5'};padding:6px 10px;margin-bottom:7px;border-radius:0 var(--radius) var(--radius) 0;cursor:pointer" onclick="openEvtDetail('${e.evtType}','${e.id||e._supaId||''}')">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div style="font-size:11px;font-weight:600">${e.titre||e.agence||'—'}</div>
          ${gcalUrl?`<a href="${gcalUrl}" target="_blank" title="Ajouter à Google Agenda" style="font-size:10px;color:#185FA5;text-decoration:none;flex-shrink:0;margin-left:6px" onclick="event.stopPropagation()">📅</a>`:''}
        </div>
...
async function loadBrevo(){
  ...
  let all=[];
  try{
    const resp=await fetch('/api/brevo-contacts?t='+Date.now());
    if(!resp.ok)throw new Error('Fichier non trouvé');
    all=await resp.json();
  }catch(e){
  ...
  // 2. Charger l'historique emails transactionnels (brevo_tracking.json)
  try{
    const resp2=await fetch('/api/brevo-tracking?t='+Date.now());
    if(resp2.ok){
...
function renderDupsTable(){
  ...
  document.getElementById('dups-tbody').innerHTML=DB.dups.map((d,i)=>{
    ...
    if(!c1||!c2)return '';
    return `<tr class="dup-row"><td><div style="font-size:11px;font-weight:600">${c1.entreprise||'—'}</div><div style="font-size:10px;color:var(--text2)">${c1.email||'—'}</div></td><td><div style="font-size:11px;font-weight:600">${c2.entreprise||'—'}</div><div style="font-size:10px;color:var(--text2)">${c2.email||'—'}</div></td><td><span class="badge b-amber">${d.type}</span></td><td><button class="btn btn-sm btn-danger" onclick="mergeDup(${i})"><i class="ti ti-git-merge"></i>Fusionner</button></td></tr>`;
  }).join('');
}
...
function mergeDup(idx){
  ...
  renderBrevoTable();renderDupsTable();
  document.getElementById('bk-dups').textContent=DB.dups.length;
  document.getElementById('bk-total').textContent=DB.contacts.length;
}

function mergeAllDups(){
  const toRemove=new Set(DB.dups.map(d=>d.i2));
  ...
  notify('✅ Tous les doublons fusionnés !');
  renderBrevoTable();renderDupsTable();
  document.getElementById('bk-dups').textContent=0;
  document.getElementById('bk-total').textContent=DB.contacts.length;
}
...
function saveDeal(){
  ...
  if(!agence){notify('⚠️ Agence requise','warn');return;}
  DB.deals.push({id:'d_'+Date.now(),agence,montant:Number(document.getElementById('d-montant').value)||0,etape:document.getElementById('d-etape').value,proba:Number(document.getElementById('d-proba').value)||0,notes:document.getElementById('d-notes').value});
...
function googleCalLink(titre, date, duree, lieu, description){
  ...
  const dureeMins = {
    '30 min':30, '1h':60, '1h30':90, '2h':120, '3h':180
  }[duree] || 60;
...
function openMissionInGoogleCalendar(idx){
  ...
  const desc  = `Type : ${m.type}\nAgence : ${m.agence}\nMontant : ${m.montant} € HT\nStatut : ${m.statut}`;
  const url   = googleCalLink(titre, m.date, '2h', m.adresse, desc);
  if(!url){ notify('Date invalide','warn'); return; }
  window.open(url, '_blank');
}
...
async function obFinish(){
  ...
  hideOnboarding();
  notify('🎉 Configuration terminée — bienvenue sur EDLConnect !');
  renderDashboard();
}
...
async function onAuthSuccess(user){
  ...
  const companyName = user.user_metadata?.company_name || localStorage.getItem('edl_co_name') || 'EDLConnect';
...
function openExtranetOrderDetail(o){
  ...
  const statutBadge = (s) => {
    ...
    if(s === 'confirmee' || s === 'importee') return '<span style="color:#185FA5;background:#E6F1FB;padding:4px 12px;border-radius:20px;font-size:12px;font-weight:600">📅 Confirmé</span>';
  ...
  body.innerHTML = `
    ...
    <div style="margin-top:16px">
      <button onclick="closeExtranetOrderDetail()" 
        style="width:100%;padding:12px;background:#185FA5;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer">
        Fermer
      </button>
    </div>`;
  ...
  barsEl.innerHTML = MONTHS.map((label, i) => {
    ...
    return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:1px">
      ...
      <div style="width:100%;display:flex;flex-direction:column;justify-content:flex-end;height:90px;gap:1px">
        ...
        ${hE?`<div style="height:${hE}%;background:#185FA5;border-radius:${(hS||hA)?'0':'2px 2px'} 0 0;min-height:2px"></div>`:''}
  ...
  try {
    ...
    if(_resaLastCount > 0 && newCount > _resaLastCount) {
      ...
      sendPushNotification(
        '📥 Nouvelle réservation EDLConnect',
...
function renderReservations(list){
  ...
  if(!list.length){
    document.getElementById('resa-tbody').innerHTML = '<tr><td colspan="8" class="empty">Aucune réservation trouvée</td></tr>';
    return;
  }
...
function confirmRdvFromReservation(id){
  ...
  document.getElementById('confirm-rdv-recap').innerHTML = `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      ...
      ${tempMission.locataires && tempMission.locataires.length > 1 ? `<tr><td style="color:var(--blue-dark);padding:2px 0">👥 Locataires</td><td>${tempMission.locataires.length} convocations seront envoyées</td></tr>` : ''}
    </table>`;
...
function openConfirmRdvModal(missionId){
  ...
  document.getElementById('confirm-rdv-recap').innerHTML = `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      ...
      ${m.locataires && m.locataires.length > 1 ? `<tr><td style="color:var(--blue-dark);padding:2px 0">👥 Locataires</td><td>${m.locataires.length} convocations seront envoyées</td></tr>` : ''}
    </table>`;
...
async function sendConfirmRdv(){
  ...
  const payload = {
    ...
    locataires: (m.locataires && m.locataires.length > 0) ? m.locataires : [],
...
function copyBookingLink(){
  ...
  const link = `https://app.edlconnect.fr/booking?agency=${agencyId}&name=${agencyName}`;
...
<div id="mobile-nav-bar">
  <div class="mnav-items">
    <button onclick="nav('dashboard');closeMobileSidebar();updateMobileNav('dashboard')" id="mnav-dashboard" class="active">
      <i class="ti ti-layout-dashboard"></i>Dashboard
    </button>
    <button onclick="nav('contacts');closeMobileSidebar();updateMobileNav('contacts')" id="mnav-contacts">
      <i class="ti ti-building-store"></i>Contacts
    </button>
    <button onclick="nav('missions');closeMobileSidebar();updateMobileNav('missions')" id="mnav-missions">
      <i class="ti ti-file-check"></i>Missions
    </button>
    <button onclick="nav('agenda');closeMobileSidebar();updateMobileNav('agenda')" id="mnav-agenda">
      <i class="ti ti-calendar"></i>Agenda
    </button>
    <button onclick="nav('compose');closeMobileSidebar();updateMobileNav('compose')" id="mnav-compose">
      <i class="ti ti-send"></i>Email
    </button>
  </div>
</div>
...
<div id="auth-screen">
  <div class="auth-card">
    <div class="auth-logo">EDLConnect</div>
    <div class="auth-sub">CRM professionnel pour experts en état des lieux</div>
    <div class="auth-tabs">
      <button class="auth-tab active" onclick="authTab('login',this)">Connexion</button>
      <button class="auth-tab" onclick="authTab('signup',this)">Créer un compte</button>
    </div>
    <div id="auth-error" class="auth-error"></div>
    <div id="auth-success" class="auth-success"></div>
    <div id="auth-login">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Email</label>
      <input type="email" id="auth-email" placeholder="contact@monentreprise.fr" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Mot de passe</label>
      <input type="password" id="auth-password" placeholder="••••••••" style="margin-bottom:20px;width:100%" onkeydown="if(event.key==='Enter')doLogin()">
      ...
      <div style="text-align:center;margin-top:12px;font-size:11px">
        <a href="#" onclick="showForgotPassword()" style="color:var(--blue)">Mot de passe oublié ?</a>
      </div>
    </div>
    <div id="auth-signup" style="display:none">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Email professionnel</label>
      <input type="email" id="signup-email" placeholder="contact@monentreprise.fr" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Mot de passe (min. 6 caractères)</label>
      <input type="password" id="signup-password" placeholder="••••••••" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Nom de votre entreprise</label>
      <input type="text" id="signup-company" placeholder="EDL IDF" style="margin-bottom:20px;width:100%" onkeydown="if(event.key==='Enter')doSignup()">
      <button class="btn btn-primary" style="width:100%;justify-content:center;padding:10px" onclick="doSignup()" id="signup-btn">
        <i class="ti ti-user-plus"></i> Créer mon compte
      </button>
    </div>
    <div style="text-align:center;margin-top:20px;font-size:10px;color:var(--text3)">Vos données sont sécurisées et isolées 🔒</div>
  </div>
</div>

<!-- ONBOARDING -->
<div id="onboarding-screen" style="display:none;position:fixed;inset:0;z-index:9998;background:linear-gradient(135deg,#f8f8f6 0%,#e8f0f8 100%);align-items:center;justify-content:center;overflow-y:auto">
  <div style="background:#fff;border-radius:16px;padding:40px;width:520px;max-width:95vw;box-shadow:0 20px 60px rgba(0,0,0,.12);margin:20px auto">
    <!-- Étapes -->
    <div style="display:flex;justify-content:center;gap:8px;margin-bottom:28px" id="ob-steps">
      <div class="ob-step ob-step-active" data-step="1" style="display:flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:var(--blue)">
        <div style="width:24px;height:24px;border-radius:50%;background:var(--blue);color:#fff;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700">1</div>
        Votre entreprise
      </div>
      <div style="color:var(--border2);font-size:16px;margin-top:2px">›</div>
      <div class="ob-step" data-step="2" style="display:flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:var(--text3)">
        <div style="width:24px;height:24px;border-radius:50%;background:var(--bg3);color:var(--text3);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700">2</div>
        Facturation
      </div>
      <div style="color:var(--border2);font-size:16px;margin-top:2px">›</div>
      <div class="ob-step" data-step="3" style="display:flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:var(--text3)">
        <div style="width:24px;height:24px;border-radius:50%;background:var(--bg3);color:var(--text3);display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700">3</div>
        Intégrations
      </div>
    </div>
    ...
    <div id="ob-page-1">
      <div style="font-size:22px;font-weight:700;color:var(--blue);margin-bottom:4px">👋 Bienvenue sur EDLConnect !</div>
      <div style="font-size:13px;color:var(--text2);margin-bottom:24px">Configurons votre espace en 3 minutes. Commençons par votre entreprise.</div>
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px;margin-top:0">Nom de votre entreprise *</label>
      <input type="text" id="ob-company" placeholder="Ex : EDL Bretagne, Check&State, Mon EDL Pro…" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Votre nom complet *</label>
      <input type="text" id="ob-name" placeholder="Ex : Marie Dupont" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Adresse du siège</label>
      <input type="text" id="ob-address" placeholder="Ex : 12 rue de la Paix, 75001 Paris" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">Téléphone professionnel</label>
      <input type="tel" id="ob-phone" placeholder="Ex : 06 12 34 56 78" style="margin-bottom:24px;width:100%">
      <button class="btn btn-primary" style="width:100%;justify-content:center;padding:12px;font-size:13px" onclick="obNext(1)">
    ...
    <div id="ob-page-2" style="display:none">
      <div style="font-size:20px;font-weight:700;color:var(--blue);margin-bottom:4px">📄 Informations de facturation</div>
      <div style="font-size:13px;color:var(--text2);margin-bottom:24px">Ces infos apparaîtront sur vos factures PDF. Vous pourrez les modifier plus tard.</div>
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px;margin-top:0">SIRET</label>
      <input type="text" id="ob-siret" placeholder="Ex : 123 456 789 00012" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">N° TVA intracommunautaire</label>
      <input type="text" id="ob-tva" placeholder="Ex : FR12 345678901" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">IBAN</label>
      <input type="text" id="ob-iban" placeholder="Ex : FR76 XXXX XXXX XXXX XXXX XXXX XXX" style="margin-bottom:12px;width:100%">
      <label style="font-size:11px;color:var(--text2);display:block;margin-bottom:3px">BIC</label>
      <input type="text" id="ob-bic" placeholder="Ex : XXXXXXXX" style="margin-bottom:24px;width:100%">
      <div style="display:flex;gap:10px">
        <button class="btn" style="flex:1;justify-content:center;padding:12px" onclick="obBack(2)">
          <i class="ti ti-arrow-left"></i> Retour
        </button>
        <button class="btn btn-primary" style="flex:2;justify-content:center;padding:12px;font-size:13px" onclick="obNext(2)">
          Continuer <i class="ti ti-arrow-right"></i>
        </button>
      </div>
    </div>
    ...
    <div id="ob-page-3" style="display:none">
      <div style="font-size:20px;font-weight:700;color:var(--blue);margin-bottom:4px">🔌 Connectez vos outils</div>
      <div style="font-size:13px;color:var(--text2);margin-bottom:24px">Optionnel — vous pourrez configurer ces clés plus tard dans Paramètres.</div>
      <div style="background:var(--blue-bg);border-radius:var(--radius);padding:12px 14px;margin-bottom:16px">
        <div style="font-size:12px;font-weight:600;color:var(--blue-text);margin-bottom:4px">📧 Brevo — Envoi d'emails</div>
        <div style="font-size:11px;color:var(--text2);margin-bottom:8px">Permet d'envoyer des emails à vos contacts directement depuis EDLConnect.</div>
        <input type="password" id="ob-brevo" placeholder="xkeysib-…" style="width:100%">
        <div style="font-size:10px;color:var(--text2);margin-top:4px">Obtenir une clé gratuite sur <a href="https://app.brevo.com/settings/keys/api" target="_blank" style="color:var(--blue)">app.brevo.com</a></div>
      </div>
      <div style="background:var(--bg2);border-radius:var(--radius);padding:12px 14px;margin-bottom:24px">
        <div style="font-size:12px;font-weight:600;color:var(--text);margin-bottom:4px">✨ Mistral IA — Rédaction emails</div>
        <div style="font-size:11px;color:var(--text2);margin-bottom:8px">Permet de générer des emails professionnels automatiquement.</div>
        <input type="password" id="ob-mistral" placeholder="Clé API Mistral…" style="width:100%">
        <div style="font-size:10px;color:var(--text2);margin-top:4px">Clé gratuite sur <a href="https://console.mistral.ai/api-keys" target="_blank" style="color:var(--blue)">console.mistral.ai</a></div>
      </div>
      <div style="display:flex;gap:10px">
        <button class="btn" style="flex:1;justify-content:center;padding:12px" onclick="obBack(3)">
          <i class="ti ti-arrow-left"></i> Retour
        </button>
        <button class="btn btn-primary" style="flex:2;justify-content:center;padding:12px;font-size:13px" onclick="obFinish()">
          <i class="ti ti-check"></i> Commencer !
        </button>
      </div>
      <div style="text-align:center;margin-top:12px">
        <button onclick="obFinish()" style="background:none;border:none;color:var(--text2);font-size:11px;cursor:pointer;text-decoration:underline">Passer cette étape →</button>
      </div>
    </div>
  </div>
</div>
